"""
Прямое сравнение ALNS-mTSP и FILO2 на 3 N=100K-flagship-инстансах.

Цель — закрыть критическое возражение: на N=100K алгоритмы сравнимы по
времени работы, и нужна прямая parallel-проверка с одинаковым time-budget
и стандартным набором FILO2-флагов:

  --optimization-seconds 350
  --routemin-iterations 15   (короткая фаза route-minimization, как
                               рекомендовано в более поздней версии FILO2)
  --seed 1

ALNS запускается с тем же time-budget=350с, --step lkh_v21_minsum, --seed 1.

Артефакт: filo2_vs_alns_n100k_results.csv  (одна строка на пару (instance, solver)).
"""
...
OUT_CSV = ROOT / 'experiments' / 'review_fixes' / 'filo2_vs_alns_n100k_results.csv'
...
# -----------------------------------------------------------------------------
# Helpers (заимствовано из run_filo2_mtsp.py)
# -----------------------------------------------------------------------------
...
def write_cvrp(coords, n, m, out_path, scale=1000, demand_scale=10):
    ...
    with open(out_path, 'w') as f:
        f.write('NAME : v21_filo2_compare\n')
        f.write('TYPE : CVRP\n')
...
def parse_filo2_solution(sol_path, coords, n_total):
    """Читает FILO2 .vrp solution-файл, возвращает routes как List[List[int]]."""
    routes = []
    ...
    for ln in lines:
        if ln.startswith('Route'):
            ...
            # FILO2: 1-based customer indices (depot=1)
            # Convert to 0-based with depot=0
...
def run_filo2(inst_filename, time_s, seed):
    ...
    tmp = tempfile.mkdtemp(prefix='filo2_compare_', dir=str(ROOT / 'build'))
    try:
...
def run_alns(inst_filename, time_s, seed):
    ...
    metadata = out_json.get('steps', [{}])[0].get('metadata') or out_json.get('metadata') or {}
    ...
    for inst in INSTANCES:
        ...
        try:
            ...
            print(f'    sum={r_alns.get("sum", 0):.0f}  max={r_alns.get("makespan", 0):.0f}  '
                  ...
                  f't={r_alns.get("t_actual_s", 0):.1f}s')
        except Exception as e:
            print(f'    ERROR: {e}')
            r_alns = {'solver': 'alns_minsum', 'instance': inst, 'valid': False, 'error': str(e)[:200]}
        rows.append(r_alns)
        ...
        try:
            ...
            if r_filo.get('valid'):
                print(f'    sum={r_filo.get("sum", 0):.0f}  max={r_filo.get("makespan", 0):.0f}  '
                      ...
                      f't={r_filo.get("t_actual_s", 0):.1f}s')
            else:
                print(f'    INVALID: {r_filo.get("error", "unknown")} t={r_filo.get("t_actual_s", 0):.1f}s')
        except Exception as e:
            print(f'    ERROR: {e}')
            r_filo = {'solver': 'filo2', 'instance': inst, 'valid': False, 'error': str(e)[:200]}
        rows.append(r_filo)

    # Write CSV
    fieldnames = ['solver', 'instance', 'valid', 'timed_out', 't_actual_s',

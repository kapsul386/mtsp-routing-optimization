// mtsp_lkh_wrapper_v18.cpp
// Successor to v17. The full v17 pipeline is preserved verbatim; v18 adds one
// extra seed source: a polar-sweep partitioning that orders clients by angle
// around the depot and greedily splits them into m balanced-cost routes
// (sweep heuristic, also borrowed from a CP-tier reference solution). The
// polar seed competes against fast/savings seeds by total sum length; the
// better candidate is kept before the snapshot to seed_minsum_routes. Toggle
// with the option polar-seed=on|off (default on).

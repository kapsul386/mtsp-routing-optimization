// v15: second of the A/B series v14--v17 on the v12 pipeline. Same parameter surface as v14
// but with finer metadata granularity and minor parameter re-tuning.
// Registered as a separate SolverFactory entry solely because some CSV artifacts reference
// this name; at runtime it is nearly indistinguishable from v14.
// Superseded by v18 / ALNS-mTSP. Registered as "lkh-wrapper-v15".
...
int DesiredClusterCountLegacyv15(int node_count, int salesman_count) {
...
ClusterModelV6 BuildLightweightClustersLegacyv15(const mtsp::Instance& inst,
...
void BuildInitialRoutesClusterAwareLegacyv15(mtsp::RouteSet& out,
...
// Repair-phase statistics for v15 (same structure as v14, separate type for a distinct
// metadata namespace in JSON output: stage_v15_insert_loop_us, etc.).
struct RouteRepairStatsV15 {
    double before_sum = 0.0;
    double after_sum = 0.0;
    int attempts = 0;
    int accepted = 0;
...
// v15: A/B variant on the v12 pipeline with minor parameter re-tuning relative to v14.
// Differs from v14 only in default parameters and finer metadata granularity.
// Registered as "lkh-wrapper-v15".
class LkhWrapperSolverv15 : public Solver {
public:
    ...
    void Solve(RouteSet& out) override {
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1200) {
            ...
            if (savings_phase_ms > 0) {
                ...
                const bool built_savings = BuildSavingsSeedRoutesV15(savings_routes,
                                                                      inst,
                                                                      savings_candidates,
                                                                      distance,
                                                                      savings_budget);
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1500) {
            ...
            if (early_repair_phase_ms > 0) {
                ...
                RouteRepairStatsV15 repair_stats;
                const bool repaired = RepairRoutesMinsumV15(repaired_routes,
                                                            local_candidates,
                                                            global_candidates,
                                                            distance,
                                                            repair_budget,
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
            const int cluster_count = forced_cluster_count_ > 0
                ...
                : DesiredClusterCountLegacyv15(node_count, inst.GetSalesmanCount());
            ...
            cluster_model = BuildLightweightClustersLegacyv15(inst, cluster_count, rng, cluster_budget);
            ...
            if (!cluster_model.clusters.empty() && !first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
                ...
                for (int attempt = 0; attempt < cluster_seed_restarts && !route_budget.ShouldStop(); ++attempt) {
                    ...
                    BuildInitialRoutesClusterAwareLegacyv15(candidate_cluster_routes,
                                                            cluster_model,
                                                            inst,
                                                            distance,
                                                            route_budget,
        ...
        if (unlimited || improve_phase_ms > 0) {
            ...
            if (filo_mode && !improve_budget.ShouldStop()) {
                ...
                while (!improve_budget.ShouldStop()) {
                    if (!OrOpt1CrossRouteSweepV15(best_minsum_routes, global_candidates, distance, improve_budget)) {
                        break;
                    }
                }
                ...
                while (!improve_budget.ShouldStop()) {
                    if (!OrOpt2CrossRouteSweepV15(best_minsum_routes, global_candidates, distance, improve_budget)) {
                        break;
                    }
                }
                best_minsum_sum = RouteSumLengthV7(best_minsum_routes, distance);
                ...
                FiloMainLoopV15(best_minsum_routes,
                                best_minsum_sum,
                                best_minsum_max,
                                best_routes,
                                best_sum,
        ...
        if (route_anneal_ms > 0 && route_anneal_iters > 0 && !best_minsum_routes.empty()) {
            ...
            RouteAnnealStatsv15 route_stats;
            SearchBudgetV5 route_anneal_budget(route_anneal_ms, 0, 32);
            const bool route_annealed = AnnealRoutesMinsumv15(annealed_routes,
                                                              global_candidates,
                                                              distance,
                                                              route_anneal_budget,
                                                              route_anneal_iters,
...
private:
    ...
    struct SavingsEdgeV15 {
        int from = 0;
        int to = 0;
        double saving = 0.0;
    };

    struct SavingsRouteV15 {
        std::vector<int> nodes;
        bool active = true;
    };

    static bool IsRouteEndpointV15(const std::vector<int>& route, int city) {
        return !route.empty() && (route.front() == city || route.back() == city);
    }

    bool MergeSavingsRoutesV15(std::vector<SavingsRouteV15>& routes,
        ...
        if (!IsRouteEndpointV15(lhs.nodes, lhs_city) || !IsRouteEndpointV15(rhs.nodes, rhs_city)) {
            return false;
        }
    ...
    struct EndpointMergeMoveGreaterV15 {
        bool operator()(const EndpointMergeMoveV15& lhs, const EndpointMergeMoveV15& rhs) const {
    ...
    static void AppendReversedV15(std::vector<int>& target, const std::vector<int>& source) {
        target.insert(target.end(), source.rbegin(), source.rend());
    }

    double MergeDeltaV15(const std::vector<int>& lhs,
    ...
    std::vector<int> ApplyOpenRouteMergeV15(const std::vector<int>& lhs,
                                            const std::vector<int>& rhs,
        ...
        if (orientation == 0) {
            ...
            merged.insert(merged.end(), lhs.begin(), lhs.end());
            AppendReversedV15(merged, rhs);
        } else if (orientation == 2) {
            AppendReversedV15(merged, lhs);
            merged.insert(merged.end(), rhs.begin(), rhs.end());
        } else {
            merged.insert(merged.end(), rhs.begin(), rhs.end());
            merged.insert(merged.end(), lhs.begin(), lhs.end());
    ...
    void PushEndpointMergeMovesV15(
        int route_idx,
        const std::vector<SavingsRouteV15>& routes,
        const std::vector<int>& route_of,
        const CandidateSets& savings_candidates,
        DistanceOracleV5& distance,
        std::priority_queue<EndpointMergeMoveV15,
                            std::vector<EndpointMergeMoveV15>,
                            EndpointMergeMoveGreaterV15>& heap) const {
        ...
        for (int endpoint : endpoints) {
            ...
            for (int neighbor : savings_candidates[static_cast<size_t>(endpoint)]) {
                ...
                if (!other.active || other.nodes.empty() ||
                    !IsRouteEndpointV15(other.nodes, neighbor)) {
                    continue;
                }
                ...
                for (int orientation = 0; orientation < 4; ++orientation) {
                    const double delta = MergeDeltaV15(lhs, rhs, orientation, distance);
    ...
    bool EndpointMergeMoveStillValidV15(const EndpointMergeMoveV15& move,
                                        const std::vector<SavingsRouteV15>& routes) const {
    ...
    bool ApplyEndpointMergeMoveV15(const EndpointMergeMoveV15& move,
                                   std::vector<SavingsRouteV15>& routes,
                                   std::vector<int>& route_of) const {
        if (!EndpointMergeMoveStillValidV15(move, routes)) {
            return false;
        }
        auto& lhs = routes[static_cast<size_t>(move.lhs_route)];
        auto& rhs = routes[static_cast<size_t>(move.rhs_route)];
        std::vector<int> merged = ApplyOpenRouteMergeV15(lhs.nodes, rhs.nodes, move.orientation);
        lhs.nodes.swap(merged);
        rhs.nodes.clear();
        rhs.active = false;
        for (int city : lhs.nodes) {
    ...
    bool ReduceSavingsRoutesByEndpointQueueV15(std::vector<SavingsRouteV15>& routes,
                                               std::vector<int>& route_of,
                                               int& active_count,
                                               int target_count,
                                               const CandidateSets& savings_candidates,
        ...
        std::priority_queue<EndpointMergeMoveV15,
                            std::vector<EndpointMergeMoveV15>,
                            EndpointMergeMoveGreaterV15> heap;
        for (int route_idx = 0; route_idx < static_cast<int>(routes.size()) && !budget.ShouldStop(); ++route_idx) {
            PushEndpointMergeMovesV15(route_idx, routes, route_of, savings_candidates, distance, heap);
        }

        int stale_pops = 0;
        while (active_count > target_count && !heap.empty() && !budget.ShouldStop()) {
            EndpointMergeMoveV15 move = heap.top();
            heap.pop();
            if (!EndpointMergeMoveStillValidV15(move, routes)) {
            ...
            if (ApplyEndpointMergeMoveV15(move, routes, route_of)) {
                --active_count;
                PushEndpointMergeMovesV15(survivor, routes, route_of, savings_candidates, distance, heap);
            }
        }
        return active_count <= target_count;
    }

    bool MergeOpenRoutesToCountV15(std::vector<std::vector<int>>& open_routes,
                                   int target_count,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t lhs = 0; lhs < open_routes.size() && !budget.ShouldStop(); ++lhs) {
                for (size_t rhs = lhs + 1; rhs < open_routes.size(); ++rhs) {
                    for (int orientation = 0; orientation < 4; ++orientation) {
                        const double delta = MergeDeltaV15(open_routes[lhs], open_routes[rhs], orientation, distance);
            ...
            open_routes[best.lhs] = ApplyOpenRouteMergeV15(open_routes[best.lhs], open_routes[best.rhs], best.orientation);
    ...
    void SplitOpenRoutesToCountV15(std::vector<std::vector<int>>& open_routes, int target_count) const {
    ...
    void CleanupOpenRoutesV15(std::vector<std::vector<int>>& open_routes,
    ...
    double OpenRoutesTotalLengthV15(const std::vector<std::vector<int>>& open_routes,
                                    DistanceOracleV5& distance) const {
        double total = 0.0;
        for (const auto& route : open_routes) {
            int prev = 0;
    ...
    struct OpenInsertionChoiceV15 {
        int route = -1;
        int after = -1;
        double delta = std::numeric_limits<double>::max();
    };

    void BuildOpenRoutePositionIndexV15(const std::vector<std::vector<int>>& open_routes,
                                        int node_count,
                                        std::vector<int>& route_of,
    ...
    void ReindexOpenRouteRangeV15(const std::vector<std::vector<int>>& open_routes,
                                  int route_idx,
                                  int start_pos,
                                  std::vector<int>& route_of,
                                  std::vector<int>& pos_in_route) const {
    ...
    double OpenInsertionDeltaV15(const std::vector<int>& route,
                                 int city,
                                 int after,
                                 DistanceOracleV5& distance) const {
    ...
    OpenInsertionChoiceV15 FindBestOpenInsertionV15(int city,
        ...
        OpenInsertionChoiceV15 best;
        OpenInsertionChoiceV15 best_overflow;
        for (const auto& [route_idx, after] : positions) {
            ...
            const double delta = OpenInsertionDeltaV15(route, city, after, distance);
    ...
    bool AbsorbSmallOpenRoutesV15(std::vector<std::vector<int>>& open_routes,
                                  int target_count,
                                  int hard_max_size,
                                  const CandidateSets& candidate_sets,
                                  int node_count,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t route_idx = 0; route_idx < open_routes.size(); ++route_idx) {
                ...
                const double length = OpenRoutesTotalLengthV15(std::vector<std::vector<int>>{open_routes[route_idx]}, distance);
            ...
            BuildOpenRoutePositionIndexV15(open_routes, node_count, route_of, pos_in_route);

            bool failed = false;
            for (int city : removed) {
                OpenInsertionChoiceV15 choice = FindBestOpenInsertionV15(city,
                                                                         open_routes,
                                                                         candidate_sets,
                                                                         route_of,
                                                                         pos_in_route,
                ...
                ReindexOpenRouteRangeV15(open_routes, choice.route, insert_pos, route_of, pos_in_route);
            }
            if (failed) {
    ...
    bool BuildSavingsSeedRoutesV15(RouteSet& out,
                                   const Instance& inst,
                                   const CandidateSets& savings_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        ...
        std::vector<SavingsRouteV15> routes(static_cast<size_t>(customers));
        ...
        std::vector<SavingsEdgeV15> savings;
        ...
        std::sort(savings.begin(), savings.end(), [](const SavingsEdgeV15& lhs, const SavingsEdgeV15& rhs) {
        ...
        for (const auto& edge : savings) {
            ...
            if (MergeSavingsRoutesV15(routes, route_of, edge.from, edge.to, hard_max_size)) {
                --active_count;
            }
        }
        ...
        if (active_count > pairwise_safe_limit && !budget.ShouldStop()) {
            ReduceSavingsRoutesByEndpointQueueV15(routes,
                                                  route_of,
                                                  active_count,
                                                  pairwise_safe_limit,
                                                  savings_candidates,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target) && !budget.ShouldStop()) {
            AbsorbSmallOpenRoutesV15(open_routes,
                                     extra_route_target,
                                     hard_max_size,
                                     savings_candidates,
                                     node_count,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target)) {
            if (!MergeOpenRoutesToCountV15(open_routes, extra_route_target, distance, budget)) {
                return false;
            }
        }
        CleanupOpenRoutesV15(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_extra_routes_count"] = std::to_string(open_routes.size());
        last_metadata_["savings_extra_routes_sum"] = std::to_string(OpenRoutesTotalLengthV15(open_routes, distance));
        if (open_routes.size() > static_cast<size_t>(m)) {
            if (!MergeOpenRoutesToCountV15(open_routes, m, distance, budget, true)) {
                return false;
            }
        }
        SplitOpenRoutesToCountV15(open_routes, m);
        CleanupOpenRoutesV15(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_exact_m_sum"] = std::to_string(OpenRoutesTotalLengthV15(open_routes, distance));
    ...
    RepairInsertChoiceV15 FindBestRepairInsertionV15(int city,
        ...
        RepairInsertChoiceV15 best;
        RepairInsertChoiceV15 best_overflow;
        for (size_t route_idx = 0; route_idx < routes.size(); ++route_idx) {
            ...
            for (size_t after : positions) {
                ...
                RepairInsertChoiceV15 choice{route_idx, after, delta, true};
    ...
    bool RepairRoutesMinsumV15(RouteSet& routes,
                               const CandidateSets& local_candidates,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               RouteRepairStatsV15& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        ...
        while (!budget.ShouldStop()) {
            ...
            bool failed = false;
            for (int city : removed) {
                RepairInsertChoiceV15 choice = FindBestRepairInsertionV15(city,
                                                                          candidate,
                                                                          global_candidates,
                                                                          route_of,
                                                                          pos_in_route,
    ...
    bool AnnealRoutesMinsumv15(RouteSet& routes,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               int max_iterations,
                               RouteAnnealStatsv15& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        const int route_count = static_cast<int>(routes.size());
    ...
    bool OrOpt1CrossRouteSweepV15(RouteSet& routes,
                                   const CandidateSets& global_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        const Instance& inst = Instance::GetInstance();
    ...
    bool OrOpt2CrossRouteSweepV15(RouteSet& routes,
                                   const CandidateSets& global_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        const Instance& inst = Instance::GetInstance();
    ...
    // Key differences from RepairRoutesMinsumV15:
    ...
    void FiloMainLoopV15(RouteSet& best_minsum,
                          double& best_minsum_sum,
                          double& best_minsum_max,
        ...
        while (!budget.ShouldStop()) {
            ...
            // Select source route proportional to route length
            std::discrete_distribution<int> route_dist(route_lengths.begin(), route_lengths.end());
            const int r_src = route_dist(filo_rng);
            if (route_sizes[static_cast<size_t>(r_src)] <= 1) { ++iter; continue; }

            // Ruin: pick ruin_size consecutive interior nodes
            const int available = route_sizes[static_cast<size_t>(r_src)];
            const int take = std::min(ruin_size, available - 1);
            if (take <= 0) { ++iter; continue; }

            std::uniform_int_distribution<int> center_dist(1, available);
            const int center = center_dist(filo_rng);
            int left = std::max(1, center - take / 2);
            if (left + take > available + 1) left = std::max(1, available + 1 - take);
            const int right = left + take;

            // Incremental ruin delta for r_src
            double ruin_delta = 0.0;
            {
                const int p_node = current[static_cast<size_t>(r_src)][static_cast<size_t>(left - 1)];
                const int n_node = current[static_cast<size_t>(r_src)][static_cast<size_t>(right)];
                ruin_delta += distance(p_node, n_node);
                ruin_delta -= distance(p_node, current[static_cast<size_t>(r_src)][static_cast<size_t>(left)]);
                ruin_delta -= distance(current[static_cast<size_t>(r_src)][static_cast<size_t>(right - 1)], n_node);
                for (int p = left; p < right - 1; ++p) {
                    ruin_delta -= distance(
                        current[static_cast<size_t>(r_src)][static_cast<size_t>(p)],
                        current[static_cast<size_t>(r_src)][static_cast<size_t>(p + 1)]);
                }
            }
            ...
            for (int p = left; p < right; ++p)
                removed.push_back(current[static_cast<size_t>(r_src)][static_cast<size_t>(p)]);

            // Work on a copy for trial
            RouteSet candidate = current;
            candidate[static_cast<size_t>(r_src)].erase(
                candidate[static_cast<size_t>(r_src)].begin() + left,
                candidate[static_cast<size_t>(r_src)].begin() + right);

            // Candidate index: copy + update r_src
            ...
            cand_sizes[static_cast<size_t>(r_src)] -= take;
            // Reindex r_src from position left (elements shifted left after erase)
            for (size_t p = static_cast<size_t>(left > 0 ? left - 1 : 0);
                 p < candidate[static_cast<size_t>(r_src)].size(); ++p) {
                const int nd = candidate[static_cast<size_t>(r_src)][p];
                if (nd > 0) {
                    cand_route_of[static_cast<size_t>(nd)] = r_src;
            ...
            touched[static_cast<size_t>(r_src)] = 1;

            double insert_delta = 0.0;
            bool failed = false;
            for (int city : removed) {
                RepairInsertChoiceV15 choice = FindBestRepairInsertionV15(
...
static bool reg_lkh_mtsp_v15 = (SolverFactory::RegisterSolver("lkh-wrapper-v15", []() {
    return std::make_unique<LkhWrapperSolverv15>();
}),
                                true);

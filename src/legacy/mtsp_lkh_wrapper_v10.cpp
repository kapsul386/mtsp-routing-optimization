// v10: MINSUM-oriented successor to v9. Retains the fast modular v9 pipeline
// (including the two-phase time limit) and adds the key difference: inter-route moves
// are accepted based on the FULL route length — exactly the MINSUM objective that is
// measured for quality. In v9 acceptance was based on an approximate 4-edge delta,
// which sometimes diverged from the true MINSUM difference.
...
// Legacy cluster count formula (inherited from v6).
// The "Legacy" name reflects that in v10 this is for backward compatibility with old tests;
// the main v10 phase uses the updated logic from BuildLightweightClustersLegacyV10.
...
// Backward-compatible cluster model construction for v10.
// Matches the v6 version with minor stability fixes for n=25K--50K (where
// v6 lost validity). Used in the v10 pipeline as a preprocessing step for the seed phase.
...
// Cluster-aware seed: assigns clusters to salesmen by estimate (cluster cost),
// sorts clusters in decreasing cost order and distributes them greedily to the least-loaded routes.
// Within each cluster the route is built via greedy nearest-neighbor.
// hard_max_size guards against overloading a route by more than route_size_slack.
...
// Statistics for a single SA run on routes. Used for metadata output
// (diagnosing how effectively the annealing phase worked).
struct RouteAnnealStatsV10 {
    double before_sum = 0.0;     // MINSUM before SA
    double after_sum = 0.0;      // MINSUM after SA
    double before_energy = 0.0;  // energy (proxy cost) before
    double after_energy = 0.0;   // energy after
    int accepted_moves = 0;      // number of accepted moves (improving + SA-non-improving)
    int best_updates = 0;        // number of best-MINSUM snapshot updates
};

// v10: main solver class. Registered as "lkh-wrapper-v10".
// Pipeline: cluster-aware seed -> POPMUSIC-fast-tour-build -> guided 2-opt -> SA with MINSUM acceptance.
// Key methodological difference: inter-route moves are accepted by delta of the true MINSUM,
// not by the approximate delta used in v9.
class LkhWrapperSolverV10 : public Solver {
public:
    // Diagnostic metadata from the last run (anneal-stats, popmusic-iters, etc.).
    std::unordered_map<std::string, std::string> GetLastMetadata() const override {
        return last_metadata_;
    }

    // CLI parameters:
    //   seed, rounds, time-budget-ms — standard;
    //   local-candidate-count / global-candidate-count — k-NN and augmented (with cluster bridges);
    //   reserve-budget-ms — reserved for the final cleanup;
    //   guided-cleanup-passes — number of final guided 2-opt passes;
    //   inter-route-batch — frequency of lazy-rerun-ILS during inter-route swaps;
    //   exact-candidate-threshold — n threshold for exact fallback (instead of grid);
    //   popmusic-solutions / popmusic-sample-size / popmusic-window — POPMUSIC parameters;
    //   route-size-slack — allowed route overload relative to target_size.
...
// KEY innovation of v10: SA on routes with MINSUM acceptance.
// At each iteration:
//   1. A random pair of routes and a pair of clients within them are selected;
//   2. The delta of the true MINSUM (based on full lengths of both routes before/after) is computed;
//   3. The move is accepted by the Metropolis rule with a T-cooling schedule.
// Unlike v9 (where acceptance was based on an approximate 4-edge delta), here acceptance
// directly corresponds to the objective function — this is the main argument for v10
// in section 5.2 of the report. Returns true if at least one move was accepted.

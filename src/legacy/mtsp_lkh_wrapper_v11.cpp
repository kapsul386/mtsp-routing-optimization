// v11: v10 plus bounded cluster-level simulated annealing before the route-construction phase.
// SA at the cluster level accepts/rejects candidate permutations between client groups;
// once the clustering has converged, the full v10 route-construction pipeline is run.
// The idea is to separate coarse client assignment from fine route improvement.
...
// Energy of a single route expressed in cluster terms. Used as a proxy in the SA loop.
struct ClusterRouteEnergyV11 {
    double bridge = 0.0;          // total inter-cluster distances (by centroid)
    double load = 0.0;            // total estimated cost of clusters
    double spread = 0.0;          // total intra-cluster spread
    double dispersion = 0.0;      // average spread between centroid pairs
    double depot_pressure = 0.0;  // maximum centroid-to-depot distance in the route
    int nodes = 0;                // total number of clients
    int outliers = 0;             // total number of outlier clients
};

// Statistics for a single route-anneal phase run (for metadata output).
struct RouteAnnealStatsV11 {
...
// Intra-cluster spread estimate (proxy for the cost of visiting all clients in a cluster).
// Formula: 0.72*radius + 0.42*avg + 0.24*std, scaled by sqrt(N), plus an outlier bonus.
// Coefficients tuned empirically to match the actual route length within a cluster.
double ClusterInternalSpreadEstimateV11(const ClusterInfoV6& cluster) {
    const double node_count = static_cast<double>(std::max<size_t>(1, cluster.members.size()));
    const double root_nodes = std::sqrt(node_count);
    const double tail_radius = std::max(cluster.avg_dist, 0.5 * cluster.radius);
    return (0.72 * cluster.radius + 0.42 * cluster.avg_dist + 0.24 * cluster.std_dist) * root_nodes +
           0.16 * static_cast<double>(cluster.outlier_count) * tail_radius;
}

// Computes the energy of a single route expressed as a set of clusters.
// Internally builds a nearest-neighbor traversal of clusters (by centroid) to estimate
// the bridge cost. Does not compute the true MINSUM; this is a fast proxy for SA.
ClusterRouteEnergyV11 EvaluateClusterRouteEnergyV11(const std::vector<int>& route,
                                                    const ClusterModelV6& model) {
    ClusterRouteEnergyV11 energy;
    if (route.empty()) {
        return energy;
    }

    std::vector<int> remaining = route;
    int previous_cluster = -1;
    while (!remaining.empty()) {
        auto best_it = remaining.begin();
        double best_score = std::numeric_limits<double>::max();
        double best_bridge = 0.0;
        for (auto it = remaining.begin(); it != remaining.end(); ++it) {
            const int cluster_id = *it;
            const auto& cluster = model.clusters[static_cast<size_t>(cluster_id)];
            const double bridge = previous_cluster < 0
                ? cluster.depot_distance
                : DistanceCoordToPointV6(model.clusters[static_cast<size_t>(previous_cluster)].centroid,
                                         cluster.centroid);
            const double score = bridge + 0.12 * cluster.depot_distance + 0.05 * cluster.estimate;
            if (score + kEps < best_score || (std::abs(score - best_score) <= kEps && cluster_id < *best_it)) {
                best_score = score;
                best_bridge = bridge;
                best_it = it;
            }
        ...
        const int chosen_cluster = *best_it;
        const auto& cluster = model.clusters[static_cast<size_t>(chosen_cluster)];
        energy.bridge += best_bridge;
        energy.load += cluster.estimate;
        energy.spread += ClusterInternalSpreadEstimateV11(cluster);
        energy.depot_pressure = std::max(energy.depot_pressure, cluster.depot_distance);
        energy.nodes += static_cast<int>(cluster.members.size());
        energy.outliers += cluster.outlier_count;
        previous_cluster = chosen_cluster;
        remaining.erase(best_it);
    }

    if (previous_cluster >= 0) {
        energy.bridge += model.clusters[static_cast<size_t>(previous_cluster)].depot_distance;
    }

    for (size_t i = 0; i < route.size(); ++i) {
        const auto& lhs = model.clusters[static_cast<size_t>(route[i])];
        for (size_t j = i + 1; j < route.size(); ++j) {
            const auto& rhs = model.clusters[static_cast<size_t>(route[j])];
            energy.dispersion += DistanceCoordToPointV6(lhs.centroid, rhs.centroid);
        }
    }
    if (route.size() > 1) {
        energy.dispersion /= static_cast<double>(route.size() - 1);
    }

    return energy;
}

// Total energy of the cluster distribution across m salesmen.
// Accounts for: total_bridge (between centroids), max_length_like (max-route balancing),
// load_imbalance, node_imbalance, outlier_imbalance, and capacity penalties (soft and hard).
// Coefficient weights (0.20 for max, 4.0 for node_imbalance, 18.0 for outlier_imbalance,
// 240.0 for soft_capacity, 1e9 for hard_capacity) were tuned empirically.
// Used as the objective in AnnealClusterAssignmentMinsumV11.
double ClusterAssignmentMinsumEnergyV11(const std::vector<std::vector<int>>& assigned_clusters,
                                        const ClusterModelV6& model,
                                        int target_size,
                                        int hard_max_size) {
    ...
    std::vector<ClusterRouteEnergyV11> routes;
    routes.reserve(assigned_clusters.size());
    ...
    for (const auto& route : assigned_clusters) {
        routes.push_back(EvaluateClusterRouteEnergyV11(route, model));
        const auto& route_energy = routes.back();
        total_bridge += route_energy.bridge;
        total_load += route_energy.load;
        total_dispersion += route_energy.dispersion;
        total_nodes += route_energy.nodes;
        total_outliers += route_energy.outliers;
    }

    const double route_count = static_cast<double>(assigned_clusters.size());
    const double avg_load = total_load / std::max(1.0, route_count);
    const double avg_nodes = static_cast<double>(total_nodes) / std::max(1.0, route_count);
    const double avg_outliers = static_cast<double>(total_outliers) / std::max(1.0, route_count);

    double max_length_like = 0.0;
    double load_imbalance = 0.0;
    double node_imbalance = 0.0;
    double outlier_imbalance = 0.0;
    double soft_capacity_penalty = 0.0;
    double hard_capacity_penalty = 0.0;
    double empty_route_penalty = 0.0;

    for (const auto& route_energy : routes) {
        const double length_like = route_energy.bridge +
                                   0.11 * route_energy.load +
                                   0.22 * route_energy.spread +
                                   0.04 * route_energy.dispersion +
                                   0.05 * route_energy.depot_pressure;
        max_length_like = std::max(max_length_like, length_like);
        load_imbalance += std::abs(route_energy.load - avg_load);
        node_imbalance += std::abs(static_cast<double>(route_energy.nodes) - avg_nodes);
        outlier_imbalance += std::abs(static_cast<double>(route_energy.outliers) - avg_outliers);
        if (route_energy.nodes == 0) {
            empty_route_penalty += 2.5e7;
        }
        if (route_energy.nodes > target_size) {
            const double over = static_cast<double>(route_energy.nodes - target_size);
            soft_capacity_penalty += over * over / static_cast<double>(std::max(1, target_size));
        }
        if (route_energy.nodes > hard_max_size) {
            const double over = static_cast<double>(route_energy.nodes - hard_max_size);
            hard_capacity_penalty += 1.0e9 * (over + 1.0);
        }
    }

    return total_bridge +
           0.20 * max_length_like +
           0.035 * total_dispersion +
           0.018 * load_imbalance +
           4.0 * node_imbalance +
           18.0 * outlier_imbalance +
           240.0 * soft_capacity_penalty +
           hard_capacity_penalty +
           empty_route_penalty;
}

// One SA step at the cluster level. Randomly selects one of 4 move types:
//   move 0--1: relocate — move a cluster from one route to another;
//   move 2: swap — exchange two clusters between different routes;
//   move 3: reverse — reverse a segment of clusters within one route.
// Returns true if the move was applied (false when there are insufficient routes).
bool ApplyAnnealClusterMoveV11(std::vector<std::vector<int>>& candidate,
                               std::mt19937& rng,
                               const ClusterModelV6& model) {
    std::vector<int> non_empty;
    non_empty.reserve(candidate.size());
    for (int route = 0; route < static_cast<int>(candidate.size()); ++route) {
        if (!candidate[static_cast<size_t>(route)].empty()) {
            non_empty.push_back(route);
        }
    }
    if (non_empty.empty()) {
        return false;
    ...
    std::uniform_int_distribution<int> non_empty_pick(0, static_cast<int>(non_empty.size()) - 1);

    if (move == 0 || move == 1) {
        const int from = non_empty[static_cast<size_t>(non_empty_pick(rng))];
        auto& from_route = candidate[static_cast<size_t>(from)];
        std::uniform_int_distribution<int> pos_dist(0, static_cast<int>(from_route.size()) - 1);
        const int pos = pos_dist(rng);
        const int cluster_id = from_route[static_cast<size_t>(pos)];
        from_route.erase(from_route.begin() + pos);

        std::uniform_int_distribution<int> route_dist(0, static_cast<int>(candidate.size()) - 1);
        int to = route_dist(rng);
        if (candidate.size() > 1 && to == from && move == 0) {
            to = (to + 1) % static_cast<int>(candidate.size());
        }
        auto& to_route = candidate[static_cast<size_t>(to)];
        std::uniform_int_distribution<int> insert_dist(0, static_cast<int>(to_route.size()));
        to_route.insert(to_route.begin() + insert_dist(rng), cluster_id);
        return true;
    }
    ...
    if (move == 2) {
        ...
        int first = non_empty[static_cast<size_t>(non_empty_pick(rng))];
        int second = non_empty[static_cast<size_t>(non_empty_pick(rng))];
        if (first == second) {
            second = non_empty[static_cast<size_t>((non_empty_pick(rng) + 1) % non_empty.size())];
        }
        auto& a = candidate[static_cast<size_t>(first)];
        auto& b = candidate[static_cast<size_t>(second)];
        if (a.empty() || b.empty()) {
            return false;
        }
        std::uniform_int_distribution<int> pos_a(0, static_cast<int>(a.size()) - 1);
        std::uniform_int_distribution<int> pos_b(0, static_cast<int>(b.size()) - 1);
        std::swap(a[static_cast<size_t>(pos_a(rng))], b[static_cast<size_t>(pos_b(rng))]);
        return true;
    }

    const int route = non_empty[static_cast<size_t>(non_empty_pick(rng))];
    auto& ordered = candidate[static_cast<size_t>(route)];
    if (ordered.size() < 2) {
        return false;
    }
    std::uniform_int_distribution<int> left_dist(0, static_cast<int>(ordered.size()) - 2);
    int left = left_dist(rng);
    std::uniform_int_distribution<int> right_dist(left + 1, static_cast<int>(ordered.size()) - 1);
    int right = right_dist(rng);
    std::reverse(ordered.begin() + left, ordered.begin() + right + 1);
    return true;
}

// KEY function of v11: SA loop at the cluster level with MINSUM energy.
// Budget: max_ms (milliseconds) or max_iterations iterations, whichever comes first.
// At each iteration:
//   1. ApplyAnnealClusterMoveV11 -> candidate;
//   2. ClusterAssignmentMinsumEnergyV11(candidate) -> cand_energy;
//   3. Accept by Metropolis rule with T_init * cooling^t.
// On acceptance, committed to assigned_clusters; otherwise rolled back.
// This is the v11 experiment (cluster-SA as a separate phase). v12 did not inherit this phase
// (see Appendix D: cluster-SA produced instability on large instances).
bool AnnealClusterAssignmentMinsumV11(std::vector<std::vector<int>>& assigned_clusters,
                                      const ClusterModelV6& model,
                                      std::mt19937& rng,
                                      SearchBudgetV5& parent_budget,
                                      int max_ms,
                                      int max_iterations,
                                      int target_size,
                                      int hard_max_size,
                                      ...
                                      double& after_energy) {
    before_energy = ClusterAssignmentMinsumEnergyV11(assigned_clusters, model, target_size, hard_max_size);
    after_energy = before_energy;
    if (assigned_clusters.empty() || model.clusters.empty() || max_ms <= 0 || max_iterations <= 0) {
        return false;
    }
    const int remaining = parent_budget.RemainingMs();
    if (remaining <= 1000) {
        return false;
    }

    const int anneal_ms = std::min(max_ms, std::max(250, remaining / 3));
    SearchBudgetV5 anneal_budget(anneal_ms, 0, 16);
    std::mt19937 anneal_rng = rng;
    auto current = assigned_clusters;
    auto best = current;
    double current_energy = before_energy;
    double best_energy = before_energy;

    const double start_temp = std::max(1.0, before_energy * 0.02);
    const double end_temp = std::max(1e-3, start_temp * 0.01);
    std::uniform_real_distribution<double> probability(0.0, 1.0);

    for (int iter = 0; iter < max_iterations && !parent_budget.ShouldStop() && !anneal_budget.ShouldStop(); ++iter) {
        if ((iter & 31) == 0) {
            parent_budget.ForceCheck();
            anneal_budget.ForceCheck();
        }

        auto candidate = current;
        if (!ApplyAnnealClusterMoveV11(candidate, anneal_rng, model)) {
            continue;
        }

        const double candidate_energy = ClusterAssignmentMinsumEnergyV11(candidate, model, target_size, hard_max_size);
        ...
        if (delta <= 0.0 || probability(anneal_rng) < std::exp(-delta / std::max(1e-9, temperature))) {
    ...
    after_energy = best_energy;
    if (best_energy + kEps < before_energy) {
...
void BuildInitialRoutesClusterAwareLegacyV11(mtsp::RouteSet& out,
                                             const ClusterModelV6& model,
                                             ...
                                             int anneal_ms,
                                             int anneal_iterations,
                                             double* anneal_before_energy,
                                             double* anneal_after_energy) {
    ...
    double before_energy = 0.0;
    double after_energy = 0.0;
    AnnealClusterAssignmentMinsumV11(assigned_clusters,
                                    model,
                                    rng,
                                    budget,
                                    anneal_ms,
                                    ...
                                    before_energy,
                                    after_energy);
    if (anneal_before_energy) {
        *anneal_before_energy = before_energy;
    }
    if (anneal_after_energy) {
        *anneal_after_energy = after_energy;
    }
...
class LkhWrapperSolverV11 : public Solver {
public:
    ...
    void Configure(const std::unordered_map<std::string, std::string>& opts) override {
        ...
        if (opts.count("segment-relocate-passes")) {
            segment_relocate_passes_ = std::max(0, std::stoi(opts.at("segment-relocate-passes")));
        }
        if (opts.count("anneal-cluster-ms")) {
            anneal_cluster_ms_ = std::max(0, std::stoi(opts.at("anneal-cluster-ms")));
        }
        if (opts.count("anneal-cluster-iters")) {
            anneal_cluster_iters_ = std::max(0, std::stoi(opts.at("anneal-cluster-iters")));
        }
    ...
    void Solve(RouteSet& out) override {
        ...
        const int configured_improve_ms = unlimited
            ? 0
            : std::clamp(improvement_budget_ms_, 1, effective_total_ms);
        const int configured_first_ms = unlimited
            ? 0
        ...
        const int node_count = inst.GetNodeCount();
        const bool large_100k_profile = LargeBudget100k(node_count);
        ...
        last_metadata_["large_100k_profile"] = large_100k_profile ? "true" : "false";
        last_metadata_["effective_local_candidates"] = std::to_string(effective_local_candidates);
        last_metadata_["effective_global_candidates"] = std::to_string(effective_global_candidates);
        last_metadata_["effective_rounds"] = std::to_string(effective_rounds);
        last_metadata_["effective_popmusic_solutions"] = std::to_string(effective_popmusic_solutions);
        last_metadata_["effective_popmusic_sample"] = std::to_string(effective_popmusic_sample);
        last_metadata_["effective_popmusic_window"] = std::to_string(effective_popmusic_window);
        last_metadata_["segment_relocate_passes"] = std::to_string(EffectiveSegmentRelocatePasses(node_count));
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
            ...
            if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1500) {
                const int enrichment_phase_ms = unlimited
                    ? 0
                    : std::min(large_100k_profile ? 110000 : (node_count >= 100000 ? 80000 : (node_count >= 50000 ? 50000 : 25000)),
            ...
            if (!cluster_model.clusters.empty() && !first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
                ...
                const int route_phase_ms = unlimited
                    ? 0
                    : std::min(large_100k_profile ? 100000 : (node_count >= 100000 ? 80000 : (node_count >= 50000 ? 50000 : 25000)),
                               std::max(1000, first_budget.RemainingMs() / 2));
                SearchBudgetV5 route_budget(route_phase_ms, 0, 16);
                double anneal_before = 0.0;
                double anneal_after = 0.0;
                BuildInitialRoutesClusterAwareLegacyV11(cluster_routes,
                                                        cluster_model,
                                                        inst,
                                                        distance,
                                                        route_budget,
                                                        rng,
                                                        route_size_slack_,
                                                        EffectiveAnnealClusterMs(node_count),
                                                        EffectiveAnnealClusterIterations(node_count),
                                                        &anneal_before,
                                                        &anneal_after);
                last_metadata_["anneal_cluster_before"] = std::to_string(anneal_before);
                last_metadata_["anneal_cluster_after"] = std::to_string(anneal_after);
                SanitizeAndCompleteRoutesV7(cluster_routes, inst, distance, route_size_slack_);
                EnsureClosedDepotRoutes(cluster_routes);
        ...
        if (unlimited || improve_phase_ms > 0) {
            ...
            if (!improve_budget.ShouldStop() && improve_budget.RemainingMs() > 1000) {
                const int richer_pop_solutions = node_count >= 100000
                    ? (large_100k_profile ? std::max(effective_popmusic_solutions, 12) : std::max(effective_popmusic_solutions, 6))
                    : std::max(effective_popmusic_solutions, 8);
                const int improve_enrichment_ms = unlimited
                    ? 0
                    : std::min(large_100k_profile ? 90000 : (node_count >= 100000 ? 50000 : (node_count >= 50000 ? 35000 : 20000)),
        ...
        if (route_anneal_ms > 0 && route_anneal_iters > 0 && !best_minsum_routes.empty()) {
            ...
            RouteAnnealStatsV11 route_stats;
            SearchBudgetV5 route_anneal_budget(route_anneal_ms, 0, 32);
            const bool route_annealed = AnnealRoutesMinsumV11(annealed_routes,
                                                              global_candidates,
                                                              distance,
                                                              route_anneal_budget,
                                                              route_anneal_iters,
...
private:
    bool LargeBudget100k(int node_count) const {
        return node_count >= 100000 && time_budget_ms_ >= 550000;
    }

    int EffectiveLocalCandidateCount(int node_count) const {
        if (LargeBudget100k(node_count)) {
            return std::clamp(std::max(local_candidate_count_, 9), 8, 10);
        }
    ...
    int EffectiveGlobalCandidateCount(int node_count, int local_count) const {
        if (LargeBudget100k(node_count)) {
            return std::max(18, std::max(global_candidate_count_, local_count + 8));
        }
        if (node_count >= 100000) {
    ...
    int EffectiveGeometricCandidateCount(int node_count, int final_candidate_count) const {
        if (LargeBudget100k(node_count)) {
            return std::max(final_candidate_count * 2 + 8, 44);
        }
        if (node_count >= 100000) {
    ...
    int EffectivePopmusicSolutions(int node_count) const {
        if (LargeBudget100k(node_count)) {
            return std::clamp(std::max(popmusic_solutions_, 12), 10, 14);
        }
        if (node_count >= 100000) {
    ...
    int EffectivePopmusicSampleSize(int node_count) const {
        if (LargeBudget100k(node_count)) {
            return std::max(popmusic_sample_size_, 64);
        }
        if (node_count >= 100000) {
    ...
    int EffectivePopmusicWindow(int node_count) const {
        if (LargeBudget100k(node_count)) {
            return std::max(popmusic_window_, 48);
        }
        if (node_count >= 100000) {
    ...
    int EffectiveAnnealClusterMs(int node_count) const {
        if (anneal_cluster_ms_ >= 0) {
            return anneal_cluster_ms_;
        }
        if (node_count >= 100000) {
            return 30000;
        }
    ...
    int EffectiveAnnealClusterIterations(int node_count) const {
        if (anneal_cluster_iters_ >= 0) {
            return anneal_cluster_iters_;
        }
        if (node_count >= 100000) {
            return 80000;
        }
    ...
    int EffectiveSegmentRelocatePasses(int node_count) const {
        if (segment_relocate_passes_ >= 0) {
            return segment_relocate_passes_;
        }
        if (LargeBudget100k(node_count)) {
            return 8;
        }
    ...
    bool AnnealRoutesMinsumV11(RouteSet& routes,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               int max_iterations,
                               RouteAnnealStatsV11& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
    ...
    bool TryMinsumSegmentRelocate(RouteSet& routes,
                                  const CandidateSets& global_candidates,
                                  DistanceOracleV5& distance,
                                  SearchBudgetV5& budget,
                                  size_t& changed_from,
        ...
        for (size_t from = 0; from < routes.size() && !budget.ShouldStop(); ++from) {
            if (routes[from].size() <= 4) {
                continue;
            }
            for (size_t i = 1; i + 2 < routes[from].size() && !budget.ShouldStop(); ++i) {
                if ((i & 255U) == 0 && budget.ForceCheck()) {
                    break;
                }
                const int first = routes[from][i];
                const int second = routes[from][i + 1];
                const int prev = routes[from][i - 1];
                const int next = routes[from][i + 2];
                const double removal = distance(prev, next) -
                                       distance(prev, first) -
                                       distance(first, second) -
                                       distance(second, next);
                ...
                for (size_t to = 0; to < routes.size(); ++to) {
                    ...
                    positions.clear();
                    positions.reserve(global_candidates[static_cast<size_t>(first)].size() +
                                      global_candidates[static_cast<size_t>(second)].size() + 2ULL);
                    positions.push_back(0);
                    positions.push_back(routes[to].size() - 2);

                    auto add_neighbor_positions = [&](int city) {
                        for (int neighbor : global_candidates[static_cast<size_t>(city)]) {
                            const int pos = indices[to].Get(neighbor);
                            if (pos >= 0 && pos + 1 < static_cast<int>(routes[to].size())) {
                                positions.push_back(static_cast<size_t>(pos));
                    ...
                    add_neighbor_positions(first);
                    add_neighbor_positions(second);

                    std::sort(positions.begin(), positions.end());
                    positions.erase(std::unique(positions.begin(), positions.end()), positions.end());

                    for (size_t after : positions) {
                        const int lhs = routes[to][after];
                        const int rhs = routes[to][after + 1];
                        const double direct_insert = distance(lhs, first) +
                                                     distance(first, second) +
                                                     distance(second, rhs) -
                                                     distance(lhs, rhs);
                        const double reverse_insert = distance(lhs, second) +
                                                      distance(second, first) +
                                                      distance(first, rhs) -
                                                      distance(lhs, rhs);
                        const double direct_delta = removal + direct_insert;
                        if (direct_delta + kEps < best_delta) {
                            best_delta = direct_delta;
                            best_from = from;
                            best_to = to;
                            best_i = i;
                            best_after = after;
                        ...
                        const double reverse_delta = removal + reverse_insert;
                        if (reverse_delta + kEps < best_delta) {
                            best_delta = reverse_delta;
                            best_from = from;
                            best_to = to;
                            best_i = i;
                            best_after = after;
        ...
        const int first = routes[best_from][best_i];
        const int second = routes[best_from][best_i + 1];
        routes[best_from].erase(routes[best_from].begin() + static_cast<std::ptrdiff_t>(best_i),
                                routes[best_from].begin() + static_cast<std::ptrdiff_t>(best_i + 2));
        auto insert_pos = routes[best_to].begin() + static_cast<std::ptrdiff_t>(best_after + 1);
        if (best_reversed) {
            insert_pos = routes[best_to].insert(insert_pos, second);
            routes[best_to].insert(insert_pos + 1, first);
        } else {
            insert_pos = routes[best_to].insert(insert_pos, first);
            routes[best_to].insert(insert_pos + 1, second);
        }
        changed_from = best_from;
        changed_to = best_to;
        return true;
    ...
    void ImproveInterRoute(RouteSet& routes,
        ...
        int segment_relocate_passes = 0;
        const int effective_segment_relocate_passes = EffectiveSegmentRelocatePasses(inst.GetNodeCount());
        while (improved && !budget.ShouldStop()) {
            ...
            if (!improved && segment_relocate_passes < effective_segment_relocate_passes) {
                size_t from = routes.size();
                size_t to = routes.size();
                if (TryMinsumSegmentRelocate(routes, global_candidates, distance, budget, from, to)) {
                    QuickRouteCleanupV5(routes[from], local_candidates, distance, inst.GetNodeCount(), guided_cleanup_passes_, budget);
                    QuickRouteCleanupV5(routes[to], local_candidates, distance, inst.GetNodeCount(), guided_cleanup_passes_, budget);
                    dirty_routes[from] = 1;
                    dirty_routes[to] = 1;
                    ++accepted_since_ils;
                    ++segment_relocate_passes;
                    improved = true;
                }
            }
    ...
    int segment_relocate_passes_ = -1;
    int anneal_cluster_ms_ = -1;
    int anneal_cluster_iters_ = -1;
...
static bool reg_lkh_mtsp_v11 = (SolverFactory::RegisterSolver("lkh-wrapper-v11", []() {
    return std::make_unique<LkhWrapperSolverV11>();
}),
                                true);

// v13: portfolio wrapper over v12 (does NOT rewrite the algorithm; instead calls v12 multiple
// times with different parameter configurations and returns the best result by MINSUM).
// Structure:
//   1. BuildStages produces 2--3 stage configurations depending on instance size n:
//      - small (n < 50K):         2 stages — filo2_savings_short, popmusic_lkh_short;
//      - medium (50K <= n < 100K): 3 stages — filo2_savings_medium, popmusic_lkh_medium, v12_balanced_medium;
//      - huge (n >= 100K):         3 stages — filo2_savings_large, popmusic_lkh_large, v12_balanced_large.
//   2. For each stage a SolverFactory::Create("lkh-wrapper-v12") call is made with unique
//      parameters (the seed is XOR-hashed via 0x9E3779B9U / 0x85EBCA6BU for diversity).
//   3. The best valid stage result by MINSUM is returned as the final answer.
// This yields globally monotone improvement across v12 runs: even if one stage worsens
// the solution, the archive retains the best snapshot across all stages.
// Registered as "lkh-wrapper-v13".
...
// Safe read of an int parameter from the Options map with a fallback default.
...
// Read an unsigned int (for seed parameters with a wide range) with a fallback default.
...
// Milliseconds remaining out of total_ms since start. Never negative.
...
// Builds an Options map for a single v12 stage run. Accepts all 17 parameters
// that v12 supports (seed, budgets, candidate counts, popmusic, savings, anneal, slack values).
// Returns std::unordered_map<string, string> for passing to v12::Configure.
...
// Plan for a single stage run: a name for reporting, v12 parameters, and the requested budget.
// Stages may override the requested budget with the actual remaining time (see Solve).
struct StagePlan {
    std::string name;
    Options options;
...
// Joins the names of completed stages with commas (for metadata output).
...
// Solver class for v13. Registered as "lkh-wrapper-v13".
// Has no algorithm of its own — runs v12 multiple times with different settings
// and returns the best MINSUM result among valid stages.
class LkhWrapperSolverv13 : public Solver {
public:
    // CLI parameters:
    //   seed                — base seed (used to derive per-stage seeds);
    //   time-budget-ms      — total time limit;
    //   reserve-budget-ms   — reserve within the total limit (for post-processing);
    //   min-stage-ms        — minimum time for a single stage (skip if below this threshold);
    //   stage-reserve-ms    — reserve for a single stage relative to its budget;
    //   large-mode          — force huge mode (default: auto-detected from n).
    ...
    // Main v13 loop:
    //   1. BuildStages produces 2--3 plans depending on node_count and the total budget;
    //   2. For each plan: call v12 with its parameters, collect metadata,
    //      compare objective with best_objective, update if better;
    //   3. Return best_routes; if no stage produced a valid result — return an empty set and status="no_valid_stage".
...
private:
    // Builds the list of stage plans based on node_count and total_ms.
    // - huge (n >= 100K or large_mode_): 3 stages with long budgets (filo2-savings, popmusic-LKH, balanced);
    // - large (50K <= n < 100K): 3 stages with medium budgets;
    // - small (n < 50K): 2 stages with short budgets (filo2-savings, popmusic-LKH).
    // The magic constants (45000, 90000, 60000, etc.) are empirical limits
    // tuned for a typical 4--8-core machine.

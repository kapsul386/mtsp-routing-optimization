// Naming note: registered as "lkh-wrapper-v21" (with hyphens).
// Do NOT confuse with the modular ALNS-mTSP architecture in src/v21/, where solvers are named
// "lkh_v21_minsum" / "lkh_v21_minsum_cap" / "lkh_v21_minmax" (with underscores;
// reported as alns_minsum, alns_cap, alns_minmax). This file is the
// closing entry of the single-file line v1..v21; superseded by the modular ALNS-mTSP
// architecture, but kept for direct comparison with v20 in result CSVs and as the
// final point in the single-file evolution.
//
// Algorithmic core: v20 + Large Neighborhood Search (LNS) ruin-and-recreate.
//
// Goal: escape the "locally good but globally suboptimal" plateaus left by the pure
// descent of v20. Key idea (FILO style; also Pisinger-Ropke ALNS):
//   1. Destroy: remove K random clients from the current best routes.
//   2. Recreate: insert each removed client at the candidate-restricted position
//      that minimises the insertion delta across all m routes.
//   3. Cleanup: candidate-based 2-opt on the m modified routes.
//   4. Accept: accept the move only on strict MINSUM improvement (hill-climber).
//      Otherwise roll back to the snapshot.
// Loop until the time budget is exhausted. Strict-improvement acceptance preserves the
// safety-net guarantee: MINSUM(v21) <= MINSUM(v20) <= MINSUM(2opt+greed).
//
// ----- v20 description (kept) -----
//
...
// ===== v21: Large-Neighborhood-Search ruin-and-recreate ======================

// KEY function of v21 single-file: one ruin-and-recreate (LNS) iteration.
// Algorithm:
//   1. Destroy: remove K random clients from the current routes.
//   2. Recreate: insert each removed client at the candidate-restricted position
//      with the minimum insertion delta across all m routes.
//   3. Cleanup: candidate-based 2-opt on the m modified routes.
// Mutates routes in place; returns the final MINSUM (or +inf for an aborted iteration).
// Acceptance logic (strict-improvement or SA-Metropolis) is handled by the caller.
// This is the phase that distinguishes single-file v21 from v20: it adds a
// ruin-and-recreate loop to the v20 pipeline to escape hill-climber local optima.
template <typename DistanceFn>
double RuinAndRecreateApplyV21(mtsp::RouteSet& routes,
                               ...
                               std::mt19937& rng,
                               SearchBudgetV5& budget) {
    const int total_clients = node_count - 1;
    if (routes.size() < 2 || K <= 0 || total_clients <= K + 1) {
        return std::numeric_limits<double>::infinity();
    }

    // 2. Select K customers via spatial cluster destroy: pick a random seed
    //    customer s, then BFS-expand through `global_candidates` to collect
    //    K customers spatially close to s. This destroys a coherent geometric
    //    cluster, which is harder to recover into the same local optimum than
    //    a uniformly random destroy (the routes around s lose their structure).
    std::vector<int> customers;
    customers.reserve(static_cast<size_t>(K) * 2 + 8);
    {
        std::uniform_int_distribution<int> seed_dist(1, total_clients);
        const int seed = seed_dist(rng);
        std::vector<char> picked(static_cast<size_t>(node_count), 0);
        std::vector<int> frontier{seed};
        picked[static_cast<size_t>(seed)] = 1;
        customers.push_back(seed);
        while (static_cast<int>(customers.size()) < K && !frontier.empty()) {
            std::vector<int> next;
            next.reserve(frontier.size() * 4);
            for (int u : frontier) {
                for (int v : global_candidates[static_cast<size_t>(u)]) {
                    if (v <= 0 || picked[static_cast<size_t>(v)]) continue;
                    picked[static_cast<size_t>(v)] = 1;
                    customers.push_back(v);
                    next.push_back(v);
                    if (static_cast<int>(customers.size()) >= K) break;
                }
                if (static_cast<int>(customers.size()) >= K) break;
            }
            frontier = std::move(next);
        }
        // If candidate-graph BFS didn't fill K (sparse candidates), top up
        // with uniformly random customers — a small admixture of random
        // perturbation makes the destroy more diverse iteration-to-iteration.
        while (static_cast<int>(customers.size()) < K) {
            std::uniform_int_distribution<int> rest(1, total_clients);
            const int c = rest(rng);
            if (picked[static_cast<size_t>(c)]) continue;
            picked[static_cast<size_t>(c)] = 1;
            customers.push_back(c);
        }
    }
    customers.resize(static_cast<size_t>(K));
    std::sort(customers.begin(), customers.end());
    std::vector<char> is_removed(static_cast<size_t>(node_count), 0);
    for (int c : customers) is_removed[static_cast<size_t>(c)] = 1;

    // 3. Destroy: rebuild each route skipping removed customers. Keep depot
    //    sentinels at front/back. Track which routes were actually modified.
    std::vector<char> route_dirty(routes.size(), 0);
    for (size_t r = 0; r < routes.size(); ++r) {
        std::vector<int> kept;
        kept.reserve(routes[r].size());
        kept.push_back(0);
        bool any_removed = false;
        for (size_t i = 1; i + 1 < routes[r].size(); ++i) {
            const int c = routes[r][i];
            if (is_removed[static_cast<size_t>(c)]) {
                any_removed = true;
                continue;
            }
            kept.push_back(c);
        ...
        kept.push_back(0);
        if (any_removed) {
            routes[r] = std::move(kept);
            route_dirty[r] = 1;
        }
    }

    // 4. Recreate: shuffle removed list, then for each customer find the
    //    cheapest insertion spot across all routes using a candidate filter.
    std::shuffle(customers.begin(), customers.end(), rng);

    std::vector<RouteIndexV5> indices;
    indices.reserve(routes.size());
    for (const auto& route : routes) {
        indices.emplace_back(node_count);
        indices.back().Build(route);
    }
    ...
    for (int c : customers) {
        ...
        size_t best_route = routes.size();
        size_t best_after = 0;
        double best_delta = std::numeric_limits<double>::max();

        for (size_t r = 0; r < routes.size(); ++r) {
            // Candidate insertion positions: neighbours of `c` already placed
            // in this route, plus the two depot-adjacent slots as fallback.
            std::vector<size_t> positions;
            positions.reserve(global_candidates[static_cast<size_t>(c)].size() + 2ULL);
            positions.push_back(0);
            if (routes[r].size() >= 2) {
                positions.push_back(routes[r].size() - 2);
            }
            for (int neighbor : global_candidates[static_cast<size_t>(c)]) {
                if (neighbor <= 0 || is_removed[static_cast<size_t>(neighbor)]) continue;
                const int pos = indices[r].Get(neighbor);
                if (pos >= 0 && pos + 1 < static_cast<int>(routes[r].size())) {
                    positions.push_back(static_cast<size_t>(pos));
                }
            }
            std::sort(positions.begin(), positions.end());
            ...
            for (size_t after : positions) {
                if (after + 1 >= routes[r].size()) continue;
                const int a = routes[r][after];
                const int b = routes[r][after + 1];
                const double delta = distance(a, c) + distance(c, b) - distance(a, b);
                if (delta + kEps < best_delta) {
                    best_delta = delta;
                    best_route = r;
                    best_after = after;
        ...
        if (best_route == routes.size()) {
            // Hard fallback: insert at depot end of the first route.
            best_route = 0;
            best_after = routes[0].size() - 2;
        }
        routes[best_route].insert(
            routes[best_route].begin() + static_cast<std::ptrdiff_t>(best_after + 1), c);
        // Rebuild the affected route's index. Cheap because routes are short.
        indices[best_route].Build(routes[best_route]);
        route_dirty[best_route] = 1;
        is_removed[static_cast<size_t>(c)] = 0;
    }

    // 5. Cleanup: candidate-based 2-opt on dirty routes (cheap), then a
    //    bounded exhaustive 2-opt sweep using the raw inst.Distance — that
    //    extra step is what lets LNS escape the candidate-list neighbourhood
    //    that Phase 8 cannot leave on its own.
    for (size_t r = 0; r < routes.size(); ++r) {
        if (!route_dirty[r] || budget.ShouldStop()) continue;
        NeighborList2OptOnRouteV19(routes[r], local_candidates, distance, node_count, budget);
    }
    // Bounded exhaustive sweep on each dirty route. Uses raw distance so it
    // matches the 2opt+greed arithmetic. Wall-time bounded by sharing budget.
    {
        const mtsp::Instance& inst = mtsp::Instance::GetInstance();
        for (size_t r = 0; r < routes.size(); ++r) {
            if (!route_dirty[r] || budget.ShouldStop()) continue;
            int passes = 0;
            Exhaustive2OptOnRouteRawV20(routes[r], inst, budget, &passes);
        }
    }

    // 6. Return new MINSUM (no acceptance decision here).
    double new_sum = 0.0;
    for (const auto& r : routes) new_sum += RouteLengthGenericV5(r, distance);
    return new_sum;
}

// ===== Solver class ==========================================================

// v21 single-file: final single-file solver of the v1..v21 line.
// Registered as "lkh-wrapper-v21". Do not confuse with the modular ALNS-mTSP architecture
// in src/v21/, whose solvers are alns_minsum, alns_cap, alns_minmax (independent modular work).
// Pipeline: v20 (safety-net + clean-hybrid) + RuinAndRecreateApplyV21 loop with
// strict-improvement acceptance. Guarantee by construction:
// MINSUM(v21) <= MINSUM(v20) <= MINSUM(2opt+greed).
class LkhWrapperSolverV21 : public mtsp::Solver {
public:
    ...
    void Configure(const std::unordered_map<std::string, std::string>& opts) override {
        ...
        if (opts.count("lns")) {
            const std::string val = opts.at("lns");
            lns_enabled_ = !(val == "0" || val == "false" || val == "no" || val == "off");
        }
    }
...
#pragma omp parallel for schedule(dynamic)
        ...
        // RESERVE the last `phase9_reserve_ms` of the budget for Phase 9 LNS.
        const int phase9_reserve_ms = (lns_enabled_ && !unlimited)
            ? std::max(3000, effective_total_ms / 5)  // 20% of total, min 3s
            : 0;
        last_metadata_["phase9_reserve_ms"] = std::to_string(phase9_reserve_ms);
        int phase8_iters = 0;
        ...
        if (!total_budget.ShouldStop()) {
            ...
            while (!total_budget.ShouldStop()) {
                const int remaining = total_budget.RemainingMs();
                if (remaining <= phase9_reserve_ms + 200) break;
                bool any = false;
        ...
        // ---- Phase 9 (v21): SA-LNS ruin-and-recreate diversification ----
        // Iterated Local Search with Simulated Annealing acceptance over a
        // ruin-and-recreate move. The WORKING state can take worse moves
        // (probabilistic acceptance) but the BEST snapshot is updated only
        // on strict MINSUM improvement, so v21 MINSUM <= v20 MINSUM holds.
        // Temperature schedule: T_start = T_frac * best_sum, geometric cooling.
        int phase9_iters = 0;
        int phase9_accepts = 0;          // SA-accepted (incl. worse)
        int phase9_best_updates = 0;     // strict-best updates
        double phase9_total_delta = 0.0; // cumulative best-improvement
        double first_trial_delta = std::numeric_limits<double>::quiet_NaN();
        double last_trial_delta = std::numeric_limits<double>::quiet_NaN();
        if (lns_enabled_ && !total_budget.ShouldStop()) {
            // K_small ~ 0.1%, K_medium ~ 0.5%, K_large ~ 2% of customers.
            const int K_small = std::max(10, (node_count - 1) / 1000);
            const int K_medium = std::max(20, (node_count - 1) / 200);
            const int K_large = std::max(40, (node_count - 1) / 50);
            std::mt19937 lns_rng(seed_ ^ 0xC2B2AE3DU);
            std::uniform_real_distribution<double> uniform01(0.0, 1.0);

            mtsp::RouteSet current = best_minsum_routes;
            double current_sum = best_minsum_sum;
            // Start hot: accept ~+1% worsening with prob ~37% (exp(-1)).
            // T_init = 0.01 * current_sum. For n=100k m=5 (sum~13M) T~130k.
            // Typical LNS delta after reinsert+2-opt is ~0.1-1% = 13-130k.
            // So initial accept rate of "small" perturbations is high.
            const double T_frac_init = 0.05;
            double T = std::max(1.0, current_sum * T_frac_init);
            const double cooling = 0.985;
            int no_best_in_a_row = 0;
            // Bound iterations so we don't loop forever on tiny n.
            const int max_iters = 5000;
            while (!total_budget.ShouldStop() && phase9_iters < max_iters) {
                if (total_budget.RemainingMs() <= 500) break;
                // Pick K: if stagnant, escalate. Random small admixture.
                int K = K_small;
                if (no_best_in_a_row >= 6) K = K_medium;
                if (no_best_in_a_row >= 12) K = K_large;
                if (uniform01(lns_rng) < 0.15) K = K_medium;  // jitter

                mtsp::RouteSet trial = current;
                const double trial_sum = RuinAndRecreateApplyV21(
                    trial, global_candidates, local_candidates,
                    distance, K, node_count, lns_rng, total_budget);
                ++phase9_iters;
                if (!std::isfinite(trial_sum)) break;  // budget aborted

                const double delta = trial_sum - current_sum;
                if (std::isnan(first_trial_delta)) first_trial_delta = delta;
                last_trial_delta = delta;
                bool accept = false;
                if (delta < 0.0) {
                    accept = true;
                } else if (T > kEps) {
                    const double prob = std::exp(-delta / T);
                    accept = (uniform01(lns_rng) < prob);
                }
                if (accept) {
                    current.swap(trial);
                    ...
                    if (current_sum + kEps < best_minsum_sum &&
                        ValidateLocalRoutes(current, node_count)) {
                        const double prev_best = best_minsum_sum;
                        best_minsum_routes = current;
                        best_minsum_sum = current_sum;
                        best = best_minsum_routes;
                        best_sum = best_minsum_sum;
                        ++phase9_best_updates;
                        phase9_total_delta += (prev_best - current_sum);
                        no_best_in_a_row = 0;
                    } else {
                        ++no_best_in_a_row;
                    }
                ...
                T *= cooling;
                if (T < 0.01) {
                    // Reheat back to start temperature so the search keeps
                    // exploring rather than freezing at the current basin.
                    T = std::max(1.0, best_minsum_sum * T_frac_init);
                    // Snap working state back to best to anchor the search.
                    current = best_minsum_routes;
                    current_sum = best_minsum_sum;
                    no_best_in_a_row = 0;
                }
            }
        }
        last_metadata_["phase9_iters"] = std::to_string(phase9_iters);
        last_metadata_["phase9_accepts"] = std::to_string(phase9_accepts);
        last_metadata_["phase9_best_updates"] = std::to_string(phase9_best_updates);
        last_metadata_["phase9_total_delta"] = std::to_string(phase9_total_delta);
        if (lns_enabled_) {
            // Report typical trial delta to debug T/K tuning.
            char buf[64];
            std::snprintf(buf, sizeof(buf), "%.3f",
                          phase9_iters > 0 ? last_trial_delta : 0.0);
            last_metadata_["phase9_last_delta"] = buf;
            std::snprintf(buf, sizeof(buf), "%.3f",
                          phase9_iters > 0 ? first_trial_delta : 0.0);
            last_metadata_["phase9_first_delta"] = buf;
        }
...
static bool reg_lkh_mtsp_v21 = (SolverFactory::RegisterSolver("lkh-wrapper-v21", []() {
    return std::make_unique<LkhWrapperSolverV21>();
}),
                                true);

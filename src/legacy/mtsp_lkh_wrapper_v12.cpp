// v12: final single-file large-scale milestone. Concludes the monolithic branch.
// Extends the v10 pipeline (not the cluster-SA experiment of v11) with:
//   - explicit CLI options for splitting the time limit across phases
//     (first-solution-budget-ms, improvement-budget-ms);
//   - MINSUM archive: every improving solution is saved, and the final return is
//     the best from the archive (not the most recent phase output);
//   - cluster-aware seed + Clarke-Wright savings + candidate enrichment.
// On small instances the gap to LKH-3 is ~21% — this was the trigger for the decision
// to rewrite the architecture in src/v21/ (the ALNS-mTSP flagship).
...
int DesiredClusterCountLegacyv12(int node_count, int salesman_count) {
...
ClusterModelV6 BuildLightweightClustersLegacyv12(const mtsp::Instance& inst,
...
void BuildInitialRoutesClusterAwareLegacyv12(mtsp::RouteSet& out,
...
// Statistics for a single RepairRoutesMinsumV12 run (metadata for JSON output).
struct RouteRepairStatsV12 {
    double before_sum = 0.0;
    double after_sum = 0.0;
    int attempts = 0;     // number of repair move attempts
    int accepted = 0;     // number of improvements actually applied
};

// v12: final single-file solver class. Registered as "lkh-wrapper-v12".
// Pipeline: cluster-aware seed -> savings-merge -> POPMUSIC-tour -> SA-anneal -> final-2-opt.
// Key methodological difference from v10/v11: an explicit MINSUM archive (snapshot of the best
// solution after each phase) guarantees that the final return is globally best by MINSUM,
// not merely the most recent phase output.
class LkhWrapperSolverv12 : public Solver {
public:
    std::unordered_map<std::string, std::string> GetLastMetadata() const override {
        return last_metadata_;
    }

    // CLI parameters for v12 (extended set compared to v10):
    //   first-solution-budget-ms / improvement-budget-ms — explicit phase time split;
    //   savings-* — Clarke-Wright savings-merge parameters;
    //   archive-* — MINSUM archive parameters (update threshold, history size);
    //   repair-* — post-anneal repair phase parameters;
    //   plus inherited from v10: cluster-*, popmusic-*, anneal-*.
    ...
    void Solve(RouteSet& out) override {
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1200) {
            ...
            if (savings_phase_ms > 0) {
                ...
                const bool built_savings = BuildSavingsSeedRoutesV12(savings_routes,
                                                                      inst,
                                                                      savings_candidates,
                                                                      distance,
                                                                      savings_budget);
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1500) {
            ...
            if (early_repair_phase_ms > 0) {
                ...
                RouteRepairStatsV12 repair_stats;
                const bool repaired = RepairRoutesMinsumV12(repaired_routes,
                                                            local_candidates,
                                                            global_candidates,
                                                            distance,
                                                            repair_budget,
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
            const int cluster_count = forced_cluster_count_ > 0
                ...
                : DesiredClusterCountLegacyv12(node_count, inst.GetSalesmanCount());
            ...
            cluster_model = BuildLightweightClustersLegacyv12(inst, cluster_count, rng, cluster_budget);
            ...
            if (!cluster_model.clusters.empty() && !first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
                ...
                for (int attempt = 0; attempt < cluster_seed_restarts && !route_budget.ShouldStop(); ++attempt) {
                    ...
                    BuildInitialRoutesClusterAwareLegacyv12(candidate_cluster_routes,
                                                            cluster_model,
                                                            inst,
                                                            distance,
                                                            route_budget,
        ...
        if (route_anneal_ms > 0 && route_anneal_iters > 0 && !best_minsum_routes.empty()) {
            ...
            RouteAnnealStatsv12 route_stats;
            SearchBudgetV5 route_anneal_budget(route_anneal_ms, 0, 32);
            const bool route_annealed = AnnealRoutesMinsumv12(annealed_routes,
                                                              global_candidates,
                                                              distance,
                                                              route_anneal_budget,
                                                              route_anneal_iters,
...
private:
    ...
    // KEY function of v12: updating the MINSUM archive.
    // If candidate is better than best_minsum_routes by MINSUM (or equal but with a smaller
    // max route), the archive is updated. This is an architectural invariant: the final return
    // is always best_minsum_routes, not the current state of the last phase. This fixes the
    // v10/v11 problem where a late phase could accidentally worsen the result.
    ...
    // Edge for the Clarke-Wright savings heuristic:
    //   savings(i, j) = d(0, i) + d(0, j) - d(i, j),
    // i.e. the saving from merging two routes [0,i,0] and [0,j,0] into one [0,i,j,0].
    struct SavingsEdgeV12 {
        int from = 0;
        int to = 0;
        double saving = 0.0;
    };

    // A candidate route in the savings phase (before the final merge into m routes).
    struct SavingsRouteV12 {
        std::vector<int> nodes;
        bool active = true;     // true while the route has not been absorbed by another
    };

    // Check: city is an endpoint of the route (either the first or the last client).
    // Only endpoints can be merged in the Clarke-Wright savings scheme.
    static bool IsRouteEndpointV12(const std::vector<int>& route, int city) {
        return !route.empty() && (route.front() == city || route.back() == city);
    }

    // Merges two savings routes at their endpoints.
    // Considers all 4 orientation combinations and the hard_max_size constraint.
    // Deactivates the absorbed route and redirects route_of pointers.
    bool MergeSavingsRoutesV12(std::vector<SavingsRouteV12>& routes,
        ...
        if (!IsRouteEndpointV12(lhs.nodes, lhs_city) || !IsRouteEndpointV12(rhs.nodes, rhs_city)) {
            return false;
        }
    ...
    // Candidate selection for savings-merge: which two routes to merge and how (orientation).
    struct MergeChoiceV12 {
        size_t lhs = 0;
        size_t rhs = 0;
        int orientation = 0;
        double delta = std::numeric_limits<double>::max();   // MINSUM change
        double score = std::numeric_limits<double>::max();   // final score (delta + tie-break)
    };

    // Endpoint-merge move: candidate entry for the EndpointMergeMoveGreaterV12 priority queue.
    struct EndpointMergeMoveV12 {
        double delta = 0.0;
        int lhs_route = -1;
        int rhs_route = -1;
        int lhs_front = 0;
    ...
    struct EndpointMergeMoveGreaterV12 {
        bool operator()(const EndpointMergeMoveV12& lhs, const EndpointMergeMoveV12& rhs) const {
    ...
    static void AppendReversedV12(std::vector<int>& target, const std::vector<int>& source) {
        target.insert(target.end(), source.rbegin(), source.rend());
    }

    double MergeDeltaV12(const std::vector<int>& lhs,
    ...
    std::vector<int> ApplyOpenRouteMergeV12(const std::vector<int>& lhs,
                                            const std::vector<int>& rhs,
        ...
        if (orientation == 0) {
            ...
            merged.insert(merged.end(), lhs.begin(), lhs.end());
            AppendReversedV12(merged, rhs);
        } else if (orientation == 2) {
            AppendReversedV12(merged, lhs);
            merged.insert(merged.end(), rhs.begin(), rhs.end());
        } else {
            merged.insert(merged.end(), rhs.begin(), rhs.end());
            merged.insert(merged.end(), lhs.begin(), lhs.end());
    ...
    void PushEndpointMergeMovesV12(
        int route_idx,
        const std::vector<SavingsRouteV12>& routes,
        const std::vector<int>& route_of,
        const CandidateSets& savings_candidates,
        DistanceOracleV5& distance,
        std::priority_queue<EndpointMergeMoveV12,
                            std::vector<EndpointMergeMoveV12>,
                            EndpointMergeMoveGreaterV12>& heap) const {
        ...
        for (int endpoint : endpoints) {
            ...
            for (int neighbor : savings_candidates[static_cast<size_t>(endpoint)]) {
                ...
                if (!other.active || other.nodes.empty() ||
                    !IsRouteEndpointV12(other.nodes, neighbor)) {
                    continue;
                }
                ...
                for (int orientation = 0; orientation < 4; ++orientation) {
                    const double delta = MergeDeltaV12(lhs, rhs, orientation, distance);
    ...
    bool EndpointMergeMoveStillValidV12(const EndpointMergeMoveV12& move,
                                        const std::vector<SavingsRouteV12>& routes) const {
    ...
    bool ApplyEndpointMergeMoveV12(const EndpointMergeMoveV12& move,
                                   std::vector<SavingsRouteV12>& routes,
                                   std::vector<int>& route_of) const {
        if (!EndpointMergeMoveStillValidV12(move, routes)) {
            return false;
        }
        auto& lhs = routes[static_cast<size_t>(move.lhs_route)];
        auto& rhs = routes[static_cast<size_t>(move.rhs_route)];
        std::vector<int> merged = ApplyOpenRouteMergeV12(lhs.nodes, rhs.nodes, move.orientation);
        lhs.nodes.swap(merged);
        rhs.nodes.clear();
        rhs.active = false;
        for (int city : lhs.nodes) {
    ...
    bool ReduceSavingsRoutesByEndpointQueueV12(std::vector<SavingsRouteV12>& routes,
                                               std::vector<int>& route_of,
                                               int& active_count,
                                               int target_count,
                                               const CandidateSets& savings_candidates,
        ...
        std::priority_queue<EndpointMergeMoveV12,
                            std::vector<EndpointMergeMoveV12>,
                            EndpointMergeMoveGreaterV12> heap;
        for (int route_idx = 0; route_idx < static_cast<int>(routes.size()) && !budget.ShouldStop(); ++route_idx) {
            PushEndpointMergeMovesV12(route_idx, routes, route_of, savings_candidates, distance, heap);
        }

        int stale_pops = 0;
        while (active_count > target_count && !heap.empty() && !budget.ShouldStop()) {
            EndpointMergeMoveV12 move = heap.top();
            heap.pop();
            if (!EndpointMergeMoveStillValidV12(move, routes)) {
            ...
            if (ApplyEndpointMergeMoveV12(move, routes, route_of)) {
                --active_count;
                PushEndpointMergeMovesV12(survivor, routes, route_of, savings_candidates, distance, heap);
            }
        }
        return active_count <= target_count;
    }

    bool MergeOpenRoutesToCountV12(std::vector<std::vector<int>>& open_routes,
                                   int target_count,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t lhs = 0; lhs < open_routes.size() && !budget.ShouldStop(); ++lhs) {
                for (size_t rhs = lhs + 1; rhs < open_routes.size(); ++rhs) {
                    for (int orientation = 0; orientation < 4; ++orientation) {
                        const double delta = MergeDeltaV12(open_routes[lhs], open_routes[rhs], orientation, distance);
            ...
            open_routes[best.lhs] = ApplyOpenRouteMergeV12(open_routes[best.lhs], open_routes[best.rhs], best.orientation);
    ...
    void SplitOpenRoutesToCountV12(std::vector<std::vector<int>>& open_routes, int target_count) const {
    ...
    void CleanupOpenRoutesV12(std::vector<std::vector<int>>& open_routes,
    ...
    double OpenRoutesTotalLengthV12(const std::vector<std::vector<int>>& open_routes,
                                    DistanceOracleV5& distance) const {
        double total = 0.0;
        for (const auto& route : open_routes) {
            int prev = 0;
    ...
    struct OpenInsertionChoiceV12 {
        int route = -1;
        int after = -1;
        double delta = std::numeric_limits<double>::max();
    };

    void BuildOpenRoutePositionIndexV12(const std::vector<std::vector<int>>& open_routes,
                                        int node_count,
                                        std::vector<int>& route_of,
    ...
    void ReindexOpenRouteRangeV12(const std::vector<std::vector<int>>& open_routes,
                                  int route_idx,
                                  int start_pos,
                                  std::vector<int>& route_of,
                                  std::vector<int>& pos_in_route) const {
    ...
    double OpenInsertionDeltaV12(const std::vector<int>& route,
                                 int city,
                                 int after,
                                 DistanceOracleV5& distance) const {
    ...
    OpenInsertionChoiceV12 FindBestOpenInsertionV12(int city,
        ...
        OpenInsertionChoiceV12 best;
        OpenInsertionChoiceV12 best_overflow;
        for (const auto& [route_idx, after] : positions) {
            ...
            const double delta = OpenInsertionDeltaV12(route, city, after, distance);
    ...
    bool AbsorbSmallOpenRoutesV12(std::vector<std::vector<int>>& open_routes,
                                  int target_count,
                                  int hard_max_size,
                                  const CandidateSets& candidate_sets,
                                  int node_count,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t route_idx = 0; route_idx < open_routes.size(); ++route_idx) {
                ...
                const double length = OpenRoutesTotalLengthV12(std::vector<std::vector<int>>{open_routes[route_idx]}, distance);
            ...
            BuildOpenRoutePositionIndexV12(open_routes, node_count, route_of, pos_in_route);

            bool failed = false;
            for (int city : removed) {
                OpenInsertionChoiceV12 choice = FindBestOpenInsertionV12(city,
                                                                         open_routes,
                                                                         candidate_sets,
                                                                         route_of,
                                                                         pos_in_route,
                ...
                ReindexOpenRouteRangeV12(open_routes, choice.route, insert_pos, route_of, pos_in_route);
            }
            if (failed) {
    ...
    // Main Clarke-Wright savings phase function.
    // Start: each client in its own route [0, c, 0]. Then:
    //   1. Compute savings(i, j) for all candidate pairs (via savings_candidates).
    //   2. Sort pairs in decreasing savings order.
    //   3. For each pair: if both clients are endpoints of their routes
    //      and merging does not exceed hard_max_size, perform the merge.
    //   4. Loop until m routes are reached or all pairs are exhausted.
    // This is a fast O(n*k log(n*k)) seed phase, efficient for large n.
    bool BuildSavingsSeedRoutesV12(RouteSet& out,
                                   const Instance& inst,
                                   const CandidateSets& savings_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        ...
        std::vector<SavingsRouteV12> routes(static_cast<size_t>(customers));
        ...
        std::vector<SavingsEdgeV12> savings;
        ...
        std::sort(savings.begin(), savings.end(), [](const SavingsEdgeV12& lhs, const SavingsEdgeV12& rhs) {
        ...
        for (const auto& edge : savings) {
            ...
            if (MergeSavingsRoutesV12(routes, route_of, edge.from, edge.to, hard_max_size)) {
                --active_count;
            }
        }
        ...
        if (active_count > pairwise_safe_limit && !budget.ShouldStop()) {
            ReduceSavingsRoutesByEndpointQueueV12(routes,
                                                  route_of,
                                                  active_count,
                                                  pairwise_safe_limit,
                                                  savings_candidates,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target) && !budget.ShouldStop()) {
            AbsorbSmallOpenRoutesV12(open_routes,
                                     extra_route_target,
                                     hard_max_size,
                                     savings_candidates,
                                     node_count,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target)) {
            if (!MergeOpenRoutesToCountV12(open_routes, extra_route_target, distance, budget)) {
                return false;
            }
        }
        CleanupOpenRoutesV12(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_extra_routes_count"] = std::to_string(open_routes.size());
        last_metadata_["savings_extra_routes_sum"] = std::to_string(OpenRoutesTotalLengthV12(open_routes, distance));
        if (open_routes.size() > static_cast<size_t>(m)) {
            if (!MergeOpenRoutesToCountV12(open_routes, m, distance, budget, true)) {
                return false;
            }
        }
        SplitOpenRoutesToCountV12(open_routes, m);
        CleanupOpenRoutesV12(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_exact_m_sum"] = std::to_string(OpenRoutesTotalLengthV12(open_routes, distance));
    ...
    RepairInsertChoiceV12 FindBestRepairInsertionV12(int city,
        ...
        RepairInsertChoiceV12 best;
        RepairInsertChoiceV12 best_overflow;
        for (size_t route_idx = 0; route_idx < routes.size(); ++route_idx) {
            ...
            for (size_t after : positions) {
                ...
                RepairInsertChoiceV12 choice{route_idx, after, delta, true};
    ...
    // Repair phase: eliminates degenerate states after annealing (e.g., when
    // several clients have pooled into a single route while others are nearly empty).
    // Strategy: on each iteration find the most overloaded route and try to move
    // its endpoint clients into underloaded routes. A move is accepted only if
    // the MINSUM delta is <= 0; otherwise the move is rolled back.
    // Contract: after Repair all routes remain valid (every client visited exactly once).
    bool RepairRoutesMinsumV12(RouteSet& routes,
                               const CandidateSets& local_candidates,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               RouteRepairStatsV12& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        ...
        while (!budget.ShouldStop()) {
            ...
            for (int city : removed) {
                ...
                RepairInsertChoiceV12 choice = FindBestRepairInsertionV12(city,
                                                                          candidate,
                                                                          global_candidates,
                                                                          route_of,
                                                                          pos_in_route,
                ...
                candidate[choice.route].insert(candidate[choice.route].begin() + static_cast<std::ptrdiff_t>(choice.after + 1), city);
    ...
    bool AnnealRoutesMinsumv12(RouteSet& routes,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               int max_iterations,
                               RouteAnnealStatsv12& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        const int route_count = static_cast<int>(routes.size());
...
static bool reg_lkh_mtsp_v12 = (SolverFactory::RegisterSolver("lkh-wrapper-v12", []() {
    return std::make_unique<LkhWrapperSolverv12>();
}),
                                true);

// v20: safety-net wrapper around the hybrid MINSUM pipeline of v19.
// Primary guarantee: v20 is NEVER worse than the 2opt+greed baseline by MINSUM.
// Motivated by a nightly benchmark incident: v19 lost to 2opt+greed
// by exactly 5.7% on n=50K, m=5 and by 0.7% on n=100K, m=5.
// Approach: always build a 2opt+greed baseline in parallel with the v19 pipeline and fall back
// to it if the v19 result is worse. By construction: MINSUM(v20-result) <= MINSUM(2opt+greed).
...
// v20: safety-net wrapper around v19. Registered as "lkh-wrapper-v20".
// Key invariant: MINSUM(v20-result) <= MINSUM(2opt+greed).
// Implementation: a 2opt+greed baseline is always built alongside the v19 pipeline; if
// the final v19 result is worse, the baseline is returned. This guarantees that v20
// never regresses relative to the simple heuristic (which happened with v19 on
// n=50K, m=5 during the nightly benchmark incident).

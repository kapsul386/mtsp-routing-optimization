// v14: first of the short A/B series v14--v17 on the v12 pipeline.
// Engineering changes:
//   - Hoisted RouteIndex inside repair (previously rebuilt O(n) on each insert);
//   - OpenMP-parallel intra-route polish with thread-local distance oracle;
//   - non-zero anneal limit for n >= 50K (was 0 before).
// v15--v17 continue fine-tuning the same parameter surface; differences are in git history.
// Superseded by v18 (polar-sweep seed) and the modular ALNS-mTSP architecture in src/v21/.
// Inter-route, savings-seed, candidate-enrichment, repair stages, and the final route-anneal
// are logically identical to v12. Registered as "lkh-wrapper-v14".
...
int DesiredClusterCountLegacyv14(int node_count, int salesman_count) {
...
ClusterModelV6 BuildLightweightClustersLegacyv14(const mtsp::Instance& inst,
...
void BuildInitialRoutesClusterAwareLegacyv14(mtsp::RouteSet& out,
...
// Extended repair-phase statistics for v14 (difference from v12: microsecond-level timings
// added for the insert-loop and cleanup-loop). These are metadata written by v14 to JSON
// for debugging the relative cost of the two repair phases.
struct RouteRepairStatsV14 {
    double before_sum = 0.0;
    double after_sum = 0.0;
    int attempts = 0;
    int accepted = 0;
    long long insert_loop_us = 0;     // total time in the ruin re-insertion loop (microseconds)
    long long cleanup_loop_us = 0;    // total time in QuickRouteCleanupV5 on affected routes
};

// v14: A/B variant on the v12 pipeline with engineering improvements (hoisted RouteIndex,
// OpenMP intra-route polish, non-zero anneal limit on large n). Registered as "lkh-wrapper-v14".
class LkhWrapperSolverv14 : public Solver {
public:
    ...
    void Solve(RouteSet& out) override {
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1200) {
            ...
            if (savings_phase_ms > 0) {
                ...
                const bool built_savings = BuildSavingsSeedRoutesV14(savings_routes,
                                                                      inst,
                                                                      savings_candidates,
                                                                      distance,
                                                                      savings_budget);
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1500) {
            ...
            if (early_repair_phase_ms > 0) {
                ...
                RouteRepairStatsV14 repair_stats;
                const bool repaired = RepairRoutesMinsumV14(repaired_routes,
                                                            local_candidates,
                                                            global_candidates,
                                                            distance,
                                                            repair_budget,
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
            const int cluster_count = forced_cluster_count_ > 0
                ...
                : DesiredClusterCountLegacyv14(node_count, inst.GetSalesmanCount());
            ...
            cluster_model = BuildLightweightClustersLegacyv14(inst, cluster_count, rng, cluster_budget);
            ...
            if (!cluster_model.clusters.empty() && !first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
                ...
                for (int attempt = 0; attempt < cluster_seed_restarts && !route_budget.ShouldStop(); ++attempt) {
                    ...
                    BuildInitialRoutesClusterAwareLegacyv14(candidate_cluster_routes,
                                                            cluster_model,
                                                            inst,
                                                            distance,
                                                            route_budget,
        ...
        if (route_anneal_ms > 0 && route_anneal_iters > 0 && !best_minsum_routes.empty()) {
            ...
            RouteAnnealStatsv14 route_stats;
            SearchBudgetV5 route_anneal_budget(route_anneal_ms, 0, 32);
            const bool route_annealed = AnnealRoutesMinsumv14(annealed_routes,
                                                              global_candidates,
                                                              distance,
                                                              route_anneal_budget,
                                                              route_anneal_iters,
...
private:
    ...
    struct SavingsEdgeV14 {
        int from = 0;
        int to = 0;
        double saving = 0.0;
    };

    struct SavingsRouteV14 {
        std::vector<int> nodes;
        bool active = true;
    };

    static bool IsRouteEndpointV14(const std::vector<int>& route, int city) {
        return !route.empty() && (route.front() == city || route.back() == city);
    }

    bool MergeSavingsRoutesV14(std::vector<SavingsRouteV14>& routes,
        ...
        if (!IsRouteEndpointV14(lhs.nodes, lhs_city) || !IsRouteEndpointV14(rhs.nodes, rhs_city)) {
            return false;
        }
    ...
    struct EndpointMergeMoveGreaterV14 {
        bool operator()(const EndpointMergeMoveV14& lhs, const EndpointMergeMoveV14& rhs) const {
    ...
    static void AppendReversedV14(std::vector<int>& target, const std::vector<int>& source) {
        target.insert(target.end(), source.rbegin(), source.rend());
    }

    double MergeDeltaV14(const std::vector<int>& lhs,
    ...
    std::vector<int> ApplyOpenRouteMergeV14(const std::vector<int>& lhs,
                                            const std::vector<int>& rhs,
        ...
        if (orientation == 0) {
            ...
            merged.insert(merged.end(), lhs.begin(), lhs.end());
            AppendReversedV14(merged, rhs);
        } else if (orientation == 2) {
            AppendReversedV14(merged, lhs);
            merged.insert(merged.end(), rhs.begin(), rhs.end());
        } else {
            merged.insert(merged.end(), rhs.begin(), rhs.end());
            merged.insert(merged.end(), lhs.begin(), lhs.end());
    ...
    void PushEndpointMergeMovesV14(
        int route_idx,
        const std::vector<SavingsRouteV14>& routes,
        const std::vector<int>& route_of,
        const CandidateSets& savings_candidates,
        DistanceOracleV5& distance,
        std::priority_queue<EndpointMergeMoveV14,
                            std::vector<EndpointMergeMoveV14>,
                            EndpointMergeMoveGreaterV14>& heap) const {
        ...
        for (int endpoint : endpoints) {
            ...
            for (int neighbor : savings_candidates[static_cast<size_t>(endpoint)]) {
                ...
                if (!other.active || other.nodes.empty() ||
                    !IsRouteEndpointV14(other.nodes, neighbor)) {
                    continue;
                }
                ...
                for (int orientation = 0; orientation < 4; ++orientation) {
                    const double delta = MergeDeltaV14(lhs, rhs, orientation, distance);
    ...
    bool EndpointMergeMoveStillValidV14(const EndpointMergeMoveV14& move,
                                        const std::vector<SavingsRouteV14>& routes) const {
    ...
    bool ApplyEndpointMergeMoveV14(const EndpointMergeMoveV14& move,
                                   std::vector<SavingsRouteV14>& routes,
                                   std::vector<int>& route_of) const {
        if (!EndpointMergeMoveStillValidV14(move, routes)) {
            return false;
        }
        auto& lhs = routes[static_cast<size_t>(move.lhs_route)];
        auto& rhs = routes[static_cast<size_t>(move.rhs_route)];
        std::vector<int> merged = ApplyOpenRouteMergeV14(lhs.nodes, rhs.nodes, move.orientation);
        lhs.nodes.swap(merged);
        rhs.nodes.clear();
        rhs.active = false;
        for (int city : lhs.nodes) {
    ...
    bool ReduceSavingsRoutesByEndpointQueueV14(std::vector<SavingsRouteV14>& routes,
                                               std::vector<int>& route_of,
                                               int& active_count,
                                               int target_count,
                                               const CandidateSets& savings_candidates,
        ...
        std::priority_queue<EndpointMergeMoveV14,
                            std::vector<EndpointMergeMoveV14>,
                            EndpointMergeMoveGreaterV14> heap;
        for (int route_idx = 0; route_idx < static_cast<int>(routes.size()) && !budget.ShouldStop(); ++route_idx) {
            PushEndpointMergeMovesV14(route_idx, routes, route_of, savings_candidates, distance, heap);
        }

        int stale_pops = 0;
        while (active_count > target_count && !heap.empty() && !budget.ShouldStop()) {
            EndpointMergeMoveV14 move = heap.top();
            heap.pop();
            if (!EndpointMergeMoveStillValidV14(move, routes)) {
            ...
            if (ApplyEndpointMergeMoveV14(move, routes, route_of)) {
                --active_count;
                PushEndpointMergeMovesV14(survivor, routes, route_of, savings_candidates, distance, heap);
            }
        }
        return active_count <= target_count;
    }

    bool MergeOpenRoutesToCountV14(std::vector<std::vector<int>>& open_routes,
                                   int target_count,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t lhs = 0; lhs < open_routes.size() && !budget.ShouldStop(); ++lhs) {
                for (size_t rhs = lhs + 1; rhs < open_routes.size(); ++rhs) {
                    for (int orientation = 0; orientation < 4; ++orientation) {
                        const double delta = MergeDeltaV14(open_routes[lhs], open_routes[rhs], orientation, distance);
            ...
            open_routes[best.lhs] = ApplyOpenRouteMergeV14(open_routes[best.lhs], open_routes[best.rhs], best.orientation);
    ...
    void SplitOpenRoutesToCountV14(std::vector<std::vector<int>>& open_routes, int target_count) const {
    ...
    void CleanupOpenRoutesV14(std::vector<std::vector<int>>& open_routes,
    ...
    double OpenRoutesTotalLengthV14(const std::vector<std::vector<int>>& open_routes,
                                    DistanceOracleV5& distance) const {
        double total = 0.0;
        for (const auto& route : open_routes) {
            int prev = 0;
    ...
    struct OpenInsertionChoiceV14 {
        int route = -1;
        int after = -1;
        double delta = std::numeric_limits<double>::max();
    };

    void BuildOpenRoutePositionIndexV14(const std::vector<std::vector<int>>& open_routes,
                                        int node_count,
                                        std::vector<int>& route_of,
    ...
    void ReindexOpenRouteRangeV14(const std::vector<std::vector<int>>& open_routes,
                                  int route_idx,
                                  int start_pos,
                                  std::vector<int>& route_of,
                                  std::vector<int>& pos_in_route) const {
    ...
    double OpenInsertionDeltaV14(const std::vector<int>& route,
                                 int city,
                                 int after,
                                 DistanceOracleV5& distance) const {
    ...
    OpenInsertionChoiceV14 FindBestOpenInsertionV14(int city,
        ...
        OpenInsertionChoiceV14 best;
        OpenInsertionChoiceV14 best_overflow;
        for (const auto& [route_idx, after] : positions) {
            ...
            const double delta = OpenInsertionDeltaV14(route, city, after, distance);
    ...
    bool AbsorbSmallOpenRoutesV14(std::vector<std::vector<int>>& open_routes,
                                  int target_count,
                                  int hard_max_size,
                                  const CandidateSets& candidate_sets,
                                  int node_count,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t route_idx = 0; route_idx < open_routes.size(); ++route_idx) {
                ...
                const double length = OpenRoutesTotalLengthV14(std::vector<std::vector<int>>{open_routes[route_idx]}, distance);
            ...
            BuildOpenRoutePositionIndexV14(open_routes, node_count, route_of, pos_in_route);

            bool failed = false;
            for (int city : removed) {
                OpenInsertionChoiceV14 choice = FindBestOpenInsertionV14(city,
                                                                         open_routes,
                                                                         candidate_sets,
                                                                         route_of,
                                                                         pos_in_route,
                ...
                ReindexOpenRouteRangeV14(open_routes, choice.route, insert_pos, route_of, pos_in_route);
            }
            if (failed) {
    ...
    bool BuildSavingsSeedRoutesV14(RouteSet& out,
                                   const Instance& inst,
                                   const CandidateSets& savings_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        ...
        std::vector<SavingsRouteV14> routes(static_cast<size_t>(customers));
        ...
        std::vector<SavingsEdgeV14> savings;
        ...
        std::sort(savings.begin(), savings.end(), [](const SavingsEdgeV14& lhs, const SavingsEdgeV14& rhs) {
        ...
        for (const auto& edge : savings) {
            ...
            if (MergeSavingsRoutesV14(routes, route_of, edge.from, edge.to, hard_max_size)) {
                --active_count;
            }
        }
        ...
        if (active_count > pairwise_safe_limit && !budget.ShouldStop()) {
            ReduceSavingsRoutesByEndpointQueueV14(routes,
                                                  route_of,
                                                  active_count,
                                                  pairwise_safe_limit,
                                                  savings_candidates,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target) && !budget.ShouldStop()) {
            AbsorbSmallOpenRoutesV14(open_routes,
                                     extra_route_target,
                                     hard_max_size,
                                     savings_candidates,
                                     node_count,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target)) {
            if (!MergeOpenRoutesToCountV14(open_routes, extra_route_target, distance, budget)) {
                return false;
            }
        }
        CleanupOpenRoutesV14(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_extra_routes_count"] = std::to_string(open_routes.size());
        last_metadata_["savings_extra_routes_sum"] = std::to_string(OpenRoutesTotalLengthV14(open_routes, distance));
        if (open_routes.size() > static_cast<size_t>(m)) {
            if (!MergeOpenRoutesToCountV14(open_routes, m, distance, budget, true)) {
                return false;
            }
        }
        SplitOpenRoutesToCountV14(open_routes, m);
        CleanupOpenRoutesV14(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_exact_m_sum"] = std::to_string(OpenRoutesTotalLengthV14(open_routes, distance));
    ...
    RepairInsertChoiceV14 FindBestRepairInsertionV14(int city,
        ...
        RepairInsertChoiceV14 best;
        RepairInsertChoiceV14 best_overflow;
        for (size_t route_idx = 0; route_idx < routes.size(); ++route_idx) {
            ...
            for (size_t after : positions) {
                ...
                RepairInsertChoiceV14 choice{route_idx, after, delta, true};
    ...
    bool RepairRoutesMinsumV14(RouteSet& routes,
                               const CandidateSets& local_candidates,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               RouteRepairStatsV14& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        ...
        while (!budget.ShouldStop()) {
            ...
            bool failed = false;
            for (int city : removed) {
                RepairInsertChoiceV14 choice = FindBestRepairInsertionV14(city,
                                                                          candidate,
                                                                          global_candidates,
                                                                          route_of,
                                                                          pos_in_route,
    ...
    bool AnnealRoutesMinsumv14(RouteSet& routes,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               int max_iterations,
                               RouteAnnealStatsv14& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        const int route_count = static_cast<int>(routes.size());
...
static bool reg_lkh_mtsp_v14 = (SolverFactory::RegisterSolver("lkh-wrapper-v14", []() {
    return std::make_unique<LkhWrapperSolverv14>();
}),
                                true);

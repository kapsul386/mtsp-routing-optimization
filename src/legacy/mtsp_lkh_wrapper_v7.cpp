// v7: conclusion of the v4--v7 large-instance line. Built on the v5/v6 infrastructure
// (SearchBudgetV5, DistanceOracleV5, RouteIndexV5, SpatialGridV5, ClusterModelV6), it adds:
//   - SimulatedAnnealClusterAssignmentV7: SA at the level of cluster-to-salesman assignment
//     (predecessor to the cluster-SA experiment in v11);
//   - BuildFastSeedRoutesV7: fast seed phase later inherited by v8--v21
//     as the BuildFastSeedRoutes infrastructure;
//   - SanitizeAndCompleteRoutesV7: validity contract (all clients visited exactly once).
// Refines the spatial-grid + cache trade-offs from v4--v6, fixes the v6 validity issues
// at n >= 25K, and provides an end-to-end working pipeline for very large n.
// MINSUM quality on small instances is below v2/v3, but v7 is specifically designed for n >= 10K.
// Registered as "lkh-wrapper-v7".
...
// Total length of all routes in a RouteSet — the primary MINSUM objective metric.
...
// Length of the longest route — the MINMAX objective. Used in acceptance criteria
// where load balance among salesmen must be monitored.
...
// Approximate cost of a route serving a set of clusters.
// Uses ClusterInfoV6::estimate (precomputed) plus distances between centroids.
// This is a fast O(k) estimate for SA: computing the true MINSUM at every swap is expensive,
// so this heuristic proxy is used at the cluster level.
...
// Energy of a single cluster assignment for SA. Computes the proxy cost via
// ApproxClusterRouteCostV7 for each salesman and combines them:
// energy = sum(approx_costs) + alpha * max(approx_cost) — balances MINSUM and MINMAX.
...
// Simulated annealing over the cluster-to-salesman assignment.
// At each iteration: (1) a random cluster is picked and moved to another salesman;
// (2) the delta energy is computed via ClusterAssignmentEnergyV7; (3) the move is accepted
// by the Metropolis rule with T_init * cooling^t. The cooling schedule depends on the budget.
// This is the predecessor of the cluster-SA experiment in v11 (where SA at the cluster level
// became a separate phase). Here it is integrated into the main pipeline.
...
// Fast seed-route construction phase. Combines the best techniques from v3--v6:
// candidate-list filtering, lookahead weights, depot-aware seeds. The V7 name is kept
// because this function is reused in v8--v21 as the standard seed infrastructure
// (see src/v21/core/07_seed_routes.hpp — the descendant of the same idea).
// Parameters route_size_slack_, lookahead_weight_, depot_weight_ are CLI overrides.
template <typename DistanceFn>
...
// Route validity contract: after any phase, a call to SanitizeAndCompleteRoutesV7
// guarantees that all clients are visited exactly once and every route is closed at the depot.
// On early termination (budget cancellation), remaining unvisited clients are assigned
// to the nearest route via a cheap heuristic. route_size_slack caps the maximum
// client count per route (protection against catastrophic imbalance).
template <typename DistanceFn>
...
class LkhWrapperSolverV7 : public Solver {
public:
...
static bool reg_lkh_mtsp_v7 = (SolverFactory::RegisterSolver("lkh-wrapper-v7", []() {
    return std::make_unique<LkhWrapperSolverV7>();
}),
                               true);

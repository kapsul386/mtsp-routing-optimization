// v18: successor to v17. The full v17 pipeline is preserved unchanged; v18 adds one
// additional source of initial solutions — polar-sweep partitioning, which orders clients
// by angle around the depot and greedily partitions them into m routes of comparable cost
// (classic sweep heuristic). The polar seed competes with the fast/savings seeds by total
// length; the winner is stored in the seed_minsum_routes snapshot.
// Enabled via option polar-seed=on|off (default: on).
...
// KEY function of v18: building the seed solution via polar-sweep partitioning.
// Algorithm (classic sweep heuristic):
//   1. For each client compute angle = atan2(dy, dx) relative to the depot (vertex 0).
//   2. Sort clients by angle (ties broken by radial distance).
//   3. Split the full sorted list into m consecutive segments of comparable "cost"
//      (cost approximated by nearest-neighbor edge weight + depot-entry corrections).
//      This is an equi-cost split, not equi-count.
//   4. Each segment becomes the initial route of one salesman.
// The resulting seed competes with the fast/savings seeds in the main pipeline; if it is
// better by MINSUM, the snapshot is updated.
mtsp::RouteSet BuildPolarSweepSeedV18(const mtsp::Instance& inst) {
    const int n = inst.GetNodeCount();
    const int m = std::max(1, inst.GetSalesmanCount());
    const auto& coords = inst.GetCoords();

    mtsp::RouteSet routes(static_cast<size_t>(m));
    if (n <= 1) {
        return routes;
    }
    ...
    auto edge = [&](int a, int b) {
        const double dx = coords[static_cast<size_t>(a)].first - coords[static_cast<size_t>(b)].first;
        const double dy = coords[static_cast<size_t>(a)].second - coords[static_cast<size_t>(b)].second;
        return std::sqrt(dx * dx + dy * dy);
    };

    const int clients = static_cast<int>(order.size());
    if (clients == 0) {
        return routes;
    }
    ...
    for (int i = 1; i < clients; ++i) {
        approx_total += edge(order[static_cast<size_t>(i - 1)], order[static_cast<size_t>(i)]);
    }
    approx_total += edge(0, order.front()) + edge(order.back(), 0);

    constexpr double kTinyEps = 1e-12;
    double remaining = std::max(approx_total, kTinyEps);

    int pos = 0;
    for (int r = 0; r < m && pos < clients; ++r) {
        const int rem_routes = m - r;
        const int rem_cities = clients - pos;
        const int min_take = rem_cities >= rem_routes ? 1 : 0;
        const double target = remaining / rem_routes;
        double accum = 0.0;
        const int start = pos;

        while (pos < clients) {
            const int left_cities = clients - (pos + 1);
            const int left_routes = m - (r + 1);
            if (left_cities < left_routes) {
                break;
            }
            double add;
            if (pos == start) {
                add = 2.0 * edge(0, order[static_cast<size_t>(pos)]);
            } else {
                add = edge(order[static_cast<size_t>(pos - 1)], order[static_cast<size_t>(pos)])
                    + edge(order[static_cast<size_t>(pos)], 0)
                    - edge(order[static_cast<size_t>(pos - 1)], 0);
            }
            if (static_cast<int>(routes[static_cast<size_t>(r)].size()) >= min_take
                && accum + add > target
                && left_cities >= left_routes) {
                break;
            }
            routes[static_cast<size_t>(r)].push_back(order[static_cast<size_t>(pos)]);
            accum += add;
            ++pos;
        }

        remaining = std::max(0.0, remaining - accum);
    }
...
int DesiredClusterCountLegacyv17(int node_count, int salesman_count) {
...
ClusterModelV6 BuildLightweightClustersLegacyv17(const mtsp::Instance& inst,
...
void BuildInitialRoutesClusterAwareLegacyv17(mtsp::RouteSet& out,
...
// v18: v17 pipeline + polar-sweep seed as an additional source of initial solutions.
// Registered as "lkh-wrapper-v18".
class LkhWrapperSolverv18 : public Solver {
public:
    std::unordered_map<std::string, std::string> GetLastMetadata() const override {
        return last_metadata_;
    }

    // CLI parameters (in addition to v12/v17):
    //   polar-seed=on|off (default on) — enable polar-sweep as one of the seed candidates;
    //   omp-polish=on|off             — enable OpenMP-parallel polish;
    //   threads=N                     — explicit number of OpenMP threads (0 = autotune).
    ...
    void Solve(RouteSet& out) override {
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1200) {
            ...
            if (savings_phase_ms > 0) {
                ...
                const bool built_savings = BuildSavingsSeedRoutesV17(savings_routes,
                                                                      inst,
                                                                      savings_candidates,
                                                                      distance,
                                                                      savings_budget);
        ...
        if (polar_seed_enabled_ && !first_budget.ShouldStop()) {
            mtsp::RouteSet polar_routes = BuildPolarSweepSeedV18(inst);
            SanitizeAndCompleteRoutesV7(polar_routes,
                                        inst,
                                        distance,
                                        std::max(route_size_slack_, route_repair_size_slack_));
            EnsureClosedDepotRoutes(polar_routes);
            SanitizeAndCompleteRoutesV7(out, inst, distance, route_size_slack_);
            EnsureClosedDepotRoutes(out);
            const double polar_sum = RouteSumLengthV7(polar_routes, distance);
            const double current_sum = RouteSumLengthV7(out, distance);
            last_metadata_["polar_seed_sum"] = std::to_string(polar_sum);
            last_metadata_["polar_seed_current_sum"] = std::to_string(current_sum);
            if (polar_sum + kEps < current_sum) {
                out.swap(polar_routes);
                last_metadata_["polar_seed_selected"] = "true";
            } else {
                last_metadata_["polar_seed_selected"] = "false";
            }
        } else {
            last_metadata_["polar_seed_selected"] = "skipped";
        }
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1500) {
            ...
            if (early_repair_phase_ms > 0) {
                ...
                RouteRepairStatsV17 repair_stats;
                const bool repaired = RepairRoutesMinsumV17(repaired_routes,
                                                            local_candidates,
                                                            global_candidates,
                                                            distance,
                                                            repair_budget,
        ...
        if (!first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
            const int cluster_count = forced_cluster_count_ > 0
                ...
                : DesiredClusterCountLegacyv17(node_count, inst.GetSalesmanCount());
            ...
            cluster_model = BuildLightweightClustersLegacyv17(inst, cluster_count, rng, cluster_budget);
            ...
            if (!cluster_model.clusters.empty() && !first_budget.ShouldStop() && first_budget.RemainingMs() > 1000) {
                ...
                for (int attempt = 0; attempt < cluster_seed_restarts && !route_budget.ShouldStop(); ++attempt) {
                    ...
                    BuildInitialRoutesClusterAwareLegacyv17(candidate_cluster_routes,
                                                            cluster_model,
                                                            inst,
                                                            distance,
                                                            route_budget,
        ...
        if (unlimited || improve_phase_ms > 0) {
            ...
            if (filo_mode && !improve_budget.ShouldStop()) {
                ...
                while (!improve_budget.ShouldStop()) {
                    if (!OrOpt1CrossRouteSweepV17(best_minsum_routes, global_candidates, distance, improve_budget)) {
                        break;
                    }
                }
                ...
                while (!improve_budget.ShouldStop()) {
                    if (!OrOpt2CrossRouteSweepV17(best_minsum_routes, global_candidates, distance, improve_budget)) {
                        break;
                    }
                }
                best_minsum_sum = RouteSumLengthV7(best_minsum_routes, distance);
                ...
                // Exchange-1-1 cross-route sweep: greedy first-improving swap of single nodes.
                // Complements OrOpt1/OrOpt2 by exploring size-preserving moves.
                const double before_ex11 = best_minsum_sum;
                while (!improve_budget.ShouldStop()) {
                    if (!ExchangeOneOneCrossRouteSweepV17(best_minsum_routes, global_candidates, distance, improve_budget)) {
                        break;
                    }
                }
                best_minsum_sum = RouteSumLengthV7(best_minsum_routes, distance);
                best_minsum_max = MaxRouteLengthV7(best_minsum_routes, distance);
                last_metadata_["exchange11_delta"] = std::to_string(before_ex11 - best_minsum_sum);
                ...
                FiloMainLoopV17(best_minsum_routes,
                                best_minsum_sum,
                                best_minsum_max,
                                best_routes,
                                best_sum,
        ...
        if (route_anneal_ms > 0 && route_anneal_iters > 0 && !best_minsum_routes.empty()) {
            ...
            RouteAnnealStatsv17 route_stats;
            SearchBudgetV5 route_anneal_budget(route_anneal_ms, 0, 32);
            const bool route_annealed = AnnealRoutesMinsumv17(annealed_routes,
                                                              global_candidates,
                                                              distance,
                                                              route_anneal_budget,
                                                              route_anneal_iters,
...
private:
    ...
    struct SavingsEdgeV17 {
        int from = 0;
        int to = 0;
        double saving = 0.0;
    };

    struct SavingsRouteV17 {
        std::vector<int> nodes;
        bool active = true;
    };

    static bool IsRouteEndpointV17(const std::vector<int>& route, int city) {
        return !route.empty() && (route.front() == city || route.back() == city);
    }

    bool MergeSavingsRoutesV17(std::vector<SavingsRouteV17>& routes,
        ...
        if (!IsRouteEndpointV17(lhs.nodes, lhs_city) || !IsRouteEndpointV17(rhs.nodes, rhs_city)) {
            return false;
        }
    ...
    struct EndpointMergeMoveGreaterV17 {
        bool operator()(const EndpointMergeMoveV17& lhs, const EndpointMergeMoveV17& rhs) const {
    ...
    static void AppendReversedV17(std::vector<int>& target, const std::vector<int>& source) {
        target.insert(target.end(), source.rbegin(), source.rend());
    }

    double MergeDeltaV17(const std::vector<int>& lhs,
    ...
    std::vector<int> ApplyOpenRouteMergeV17(const std::vector<int>& lhs,
                                            const std::vector<int>& rhs,
        ...
        if (orientation == 0) {
            ...
            merged.insert(merged.end(), lhs.begin(), lhs.end());
            AppendReversedV17(merged, rhs);
        } else if (orientation == 2) {
            AppendReversedV17(merged, lhs);
            merged.insert(merged.end(), rhs.begin(), rhs.end());
        } else {
            merged.insert(merged.end(), rhs.begin(), rhs.end());
            merged.insert(merged.end(), lhs.begin(), lhs.end());
    ...
    void PushEndpointMergeMovesV17(
        int route_idx,
        const std::vector<SavingsRouteV17>& routes,
        const std::vector<int>& route_of,
        const CandidateSets& savings_candidates,
        DistanceOracleV5& distance,
        std::priority_queue<EndpointMergeMoveV17,
                            std::vector<EndpointMergeMoveV17>,
                            EndpointMergeMoveGreaterV17>& heap) const {
        ...
        for (int endpoint : endpoints) {
            ...
            for (int neighbor : savings_candidates[static_cast<size_t>(endpoint)]) {
                ...
                if (!other.active || other.nodes.empty() ||
                    !IsRouteEndpointV17(other.nodes, neighbor)) {
                    continue;
                }
                ...
                for (int orientation = 0; orientation < 4; ++orientation) {
                    const double delta = MergeDeltaV17(lhs, rhs, orientation, distance);
    ...
    bool EndpointMergeMoveStillValidV17(const EndpointMergeMoveV17& move,
                                        const std::vector<SavingsRouteV17>& routes) const {
    ...
    bool ApplyEndpointMergeMoveV17(const EndpointMergeMoveV17& move,
                                   std::vector<SavingsRouteV17>& routes,
                                   std::vector<int>& route_of) const {
        if (!EndpointMergeMoveStillValidV17(move, routes)) {
            return false;
        }
        auto& lhs = routes[static_cast<size_t>(move.lhs_route)];
        auto& rhs = routes[static_cast<size_t>(move.rhs_route)];
        std::vector<int> merged = ApplyOpenRouteMergeV17(lhs.nodes, rhs.nodes, move.orientation);
        lhs.nodes.swap(merged);
        rhs.nodes.clear();
        rhs.active = false;
        for (int city : lhs.nodes) {
    ...
    bool ReduceSavingsRoutesByEndpointQueueV17(std::vector<SavingsRouteV17>& routes,
                                               std::vector<int>& route_of,
                                               int& active_count,
                                               int target_count,
                                               const CandidateSets& savings_candidates,
        ...
        std::priority_queue<EndpointMergeMoveV17,
                            std::vector<EndpointMergeMoveV17>,
                            EndpointMergeMoveGreaterV17> heap;
        for (int route_idx = 0; route_idx < static_cast<int>(routes.size()) && !budget.ShouldStop(); ++route_idx) {
            PushEndpointMergeMovesV17(route_idx, routes, route_of, savings_candidates, distance, heap);
        }

        int stale_pops = 0;
        while (active_count > target_count && !heap.empty() && !budget.ShouldStop()) {
            EndpointMergeMoveV17 move = heap.top();
            heap.pop();
            if (!EndpointMergeMoveStillValidV17(move, routes)) {
            ...
            if (ApplyEndpointMergeMoveV17(move, routes, route_of)) {
                --active_count;
                PushEndpointMergeMovesV17(survivor, routes, route_of, savings_candidates, distance, heap);
            }
        }
        return active_count <= target_count;
    }

    bool MergeOpenRoutesToCountV17(std::vector<std::vector<int>>& open_routes,
                                   int target_count,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t lhs = 0; lhs < open_routes.size() && !budget.ShouldStop(); ++lhs) {
                for (size_t rhs = lhs + 1; rhs < open_routes.size(); ++rhs) {
                    for (int orientation = 0; orientation < 4; ++orientation) {
                        const double delta = MergeDeltaV17(open_routes[lhs], open_routes[rhs], orientation, distance);
            ...
            open_routes[best.lhs] = ApplyOpenRouteMergeV17(open_routes[best.lhs], open_routes[best.rhs], best.orientation);
    ...
    void SplitOpenRoutesToCountV17(std::vector<std::vector<int>>& open_routes, int target_count) const {
    ...
    void CleanupOpenRoutesV17(std::vector<std::vector<int>>& open_routes,
    ...
    double OpenRoutesTotalLengthV17(const std::vector<std::vector<int>>& open_routes,
                                    DistanceOracleV5& distance) const {
        double total = 0.0;
        for (const auto& route : open_routes) {
            int prev = 0;
    ...
    struct OpenInsertionChoiceV17 {
        int route = -1;
        int after = -1;
        double delta = std::numeric_limits<double>::max();
    };

    void BuildOpenRoutePositionIndexV17(const std::vector<std::vector<int>>& open_routes,
                                        int node_count,
                                        std::vector<int>& route_of,
    ...
    void ReindexOpenRouteRangeV17(const std::vector<std::vector<int>>& open_routes,
                                  int route_idx,
                                  int start_pos,
                                  std::vector<int>& route_of,
                                  std::vector<int>& pos_in_route) const {
    ...
    double OpenInsertionDeltaV17(const std::vector<int>& route,
                                 int city,
                                 int after,
                                 DistanceOracleV5& distance) const {
    ...
    OpenInsertionChoiceV17 FindBestOpenInsertionV17(int city,
        ...
        OpenInsertionChoiceV17 best;
        OpenInsertionChoiceV17 best_overflow;
        for (const auto& [route_idx, after] : positions) {
            ...
            const double delta = OpenInsertionDeltaV17(route, city, after, distance);
    ...
    bool AbsorbSmallOpenRoutesV17(std::vector<std::vector<int>>& open_routes,
                                  int target_count,
                                  int hard_max_size,
                                  const CandidateSets& candidate_sets,
                                  int node_count,
        ...
        while (open_routes.size() > static_cast<size_t>(target_count) && !budget.ShouldStop()) {
            ...
            for (size_t route_idx = 0; route_idx < open_routes.size(); ++route_idx) {
                ...
                const double length = OpenRoutesTotalLengthV17(std::vector<std::vector<int>>{open_routes[route_idx]}, distance);
            ...
            BuildOpenRoutePositionIndexV17(open_routes, node_count, route_of, pos_in_route);

            bool failed = false;
            for (int city : removed) {
                OpenInsertionChoiceV17 choice = FindBestOpenInsertionV17(city,
                                                                         open_routes,
                                                                         candidate_sets,
                                                                         route_of,
                                                                         pos_in_route,
                ...
                ReindexOpenRouteRangeV17(open_routes, choice.route, insert_pos, route_of, pos_in_route);
            }
            if (failed) {
    ...
    bool BuildSavingsSeedRoutesV17(RouteSet& out,
                                   const Instance& inst,
                                   const CandidateSets& savings_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        ...
        std::vector<SavingsRouteV17> routes(static_cast<size_t>(customers));
        ...
        std::vector<SavingsEdgeV17> savings;
        ...
        std::sort(savings.begin(), savings.end(), [](const SavingsEdgeV17& lhs, const SavingsEdgeV17& rhs) {
        ...
        for (const auto& edge : savings) {
            ...
            if (MergeSavingsRoutesV17(routes, route_of, edge.from, edge.to, hard_max_size)) {
                --active_count;
            }
        }
        ...
        if (active_count > pairwise_safe_limit && !budget.ShouldStop()) {
            ReduceSavingsRoutesByEndpointQueueV17(routes,
                                                  route_of,
                                                  active_count,
                                                  pairwise_safe_limit,
                                                  savings_candidates,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target) && !budget.ShouldStop()) {
            AbsorbSmallOpenRoutesV17(open_routes,
                                     extra_route_target,
                                     hard_max_size,
                                     savings_candidates,
                                     node_count,
        ...
        if (open_routes.size() > static_cast<size_t>(extra_route_target)) {
            if (!MergeOpenRoutesToCountV17(open_routes, extra_route_target, distance, budget)) {
                return false;
            }
        }
        CleanupOpenRoutesV17(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_extra_routes_count"] = std::to_string(open_routes.size());
        last_metadata_["savings_extra_routes_sum"] = std::to_string(OpenRoutesTotalLengthV17(open_routes, distance));
        if (open_routes.size() > static_cast<size_t>(m)) {
            if (!MergeOpenRoutesToCountV17(open_routes, m, distance, budget, true)) {
                return false;
            }
        }
        SplitOpenRoutesToCountV17(open_routes, m);
        CleanupOpenRoutesV17(open_routes, savings_candidates, node_count, distance, budget);
        last_metadata_["savings_exact_m_sum"] = std::to_string(OpenRoutesTotalLengthV17(open_routes, distance));
    ...
    RepairInsertChoiceV17 FindBestRepairInsertionV17(int city,
        ...
        RepairInsertChoiceV17 best;
        RepairInsertChoiceV17 best_overflow;
        for (size_t route_idx = 0; route_idx < routes.size(); ++route_idx) {
            ...
            for (size_t after : positions) {
                ...
                RepairInsertChoiceV17 choice{route_idx, after, delta, true};
    ...
    bool RepairRoutesMinsumV17(RouteSet& routes,
                               const CandidateSets& local_candidates,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               RouteRepairStatsV17& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        ...
        while (!budget.ShouldStop()) {
            ...
            bool failed = false;
            for (int city : removed) {
                RepairInsertChoiceV17 choice = FindBestRepairInsertionV17(city,
                                                                          candidate,
                                                                          global_candidates,
                                                                          route_of,
                                                                          pos_in_route,
    ...
    bool AnnealRoutesMinsumv17(RouteSet& routes,
                               const CandidateSets& global_candidates,
                               DistanceOracleV5& distance,
                               SearchBudgetV5& budget,
                               int max_iterations,
                               RouteAnnealStatsv17& stats) const {
        const Instance& inst = Instance::GetInstance();
        const int node_count = inst.GetNodeCount();
        const int route_count = static_cast<int>(routes.size());
    ...
    bool OrOpt1CrossRouteSweepV17(RouteSet& routes,
                                   const CandidateSets& global_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        const Instance& inst = Instance::GetInstance();
    ...
    bool OrOpt2CrossRouteSweepV17(RouteSet& routes,
                                   const CandidateSets& global_candidates,
                                   DistanceOracleV5& distance,
                                   SearchBudgetV5& budget) const {
        const Instance& inst = Instance::GetInstance();
    ...
    // ------------------------------------------------------------------ //
    //  NEW in v17: Exchange-1-1 cross-route sweep                          //
    // ------------------------------------------------------------------ //
    // Greedy first-improving swap of two single nodes between different routes.
    // For each node u, checks all k-NN neighbors v in different routes.
    // Swaps u<->v if total edge cost decreases. Route sizes preserved.
    // Complements OrOpt1 (relocate-1) by exploring moves that keep route sizes balanced.
    // O(n * k) per sweep.
    bool ExchangeOneOneCrossRouteSweepV17(RouteSet& routes,
                                           const CandidateSets& global_candidates,
                                           DistanceOracleV5& distance,
                                           SearchBudgetV5& budget) const {
        const Instance& inst = Instance::GetInstance();
        ...
        while (improved && !budget.ShouldStop()) {
            improved = false;
            for (int u = 1; u < node_count && !budget.ShouldStop(); ++u) {
                const int r_u = route_of[static_cast<size_t>(u)];
                if (r_u < 0 || route_sizes[r_u] <= 1) continue;
                const size_t pos_u = pos_in_route[static_cast<size_t>(u)];
                if (pos_u == 0 || pos_u + 1 >= routes[static_cast<size_t>(r_u)].size()) continue;

                const int prev_u = routes[static_cast<size_t>(r_u)][pos_u - 1];
                const int next_u = routes[static_cast<size_t>(r_u)][pos_u + 1];
                const double old_u_edges = distance(prev_u, u) + distance(u, next_u);

                for (int v : global_candidates[static_cast<size_t>(u)]) {
                    if (v <= 0 || v >= node_count) continue;
                    const int r_v = route_of[static_cast<size_t>(v)];
                    if (r_v < 0 || r_v == r_u || route_sizes[r_v] <= 1) continue;
                    const size_t pos_v = pos_in_route[static_cast<size_t>(v)];
                    if (pos_v == 0 || pos_v + 1 >= routes[static_cast<size_t>(r_v)].size()) continue;

                    const int prev_v = routes[static_cast<size_t>(r_v)][pos_v - 1];
                    const int next_v = routes[static_cast<size_t>(r_v)][pos_v + 1];
                    const double old_v_edges = distance(prev_v, v) + distance(v, next_v);
                    const double new_u_edges = distance(prev_u, v) + distance(v, next_u);
                    const double new_v_edges = distance(prev_v, u) + distance(u, next_v);
                    const double delta = (new_u_edges + new_v_edges) - (old_u_edges + old_v_edges);

                    if (delta < -kEps) {
                        routes[static_cast<size_t>(r_u)][pos_u] = v;
                        routes[static_cast<size_t>(r_v)][pos_v] = u;
                        route_of[static_cast<size_t>(u)] = r_v;
                        route_of[static_cast<size_t>(v)] = r_u;
                        pos_in_route[static_cast<size_t>(u)] = pos_v;
                        pos_in_route[static_cast<size_t>(v)] = pos_u;
                        improved = any = true;
                        goto next_u_ex11;
                    }
                }
    ...
    // Key differences from RepairRoutesMinsumV17:
    ...
    void FiloMainLoopV17(RouteSet& best_minsum,
                          double& best_minsum_sum,
                          double& best_minsum_max,
        ...
        while (!budget.ShouldStop()) {
            ...
            bool failed = false;
            for (int city : removed) {
                RepairInsertChoiceV17 choice = FindBestRepairInsertionV17(
            ...
            if (iter % cleanup_interval == 0 && !budget.ShouldStop()) {
                ...
                // v17: Also run ExchangeOneOne cross-route sweep every 5 cleanups.
                // O(n*k) per sweep; runs ~20 times in 300s for n=100k.
                const int exchange_interval = cleanup_interval * 5;
                if (iter % exchange_interval == 0 && !budget.ShouldStop()) {
                    ExchangeOneOneCrossRouteSweepV17(current, global_candidates, distance, budget);
                }
...
static bool reg_lkh_mtsp_v18 = (SolverFactory::RegisterSolver("lkh-wrapper-v18", []() {
    return std::make_unique<LkhWrapperSolverv18>();
}),
                                true);

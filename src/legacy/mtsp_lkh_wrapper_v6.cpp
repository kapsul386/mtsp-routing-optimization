// v6: extension of v5 with POPMUSIC sampling and a cluster model.
// Built on top of the v5 infrastructure (SearchBudgetV5, DistanceOracleV5, RouteIndexV5, SpatialGridV5),
// it adds:
//   - POPMUSIC-style fast-tour sampling (BuildPopmusicTourV5, FastPopmusicPassV5)
//     to construct a single global tour before splitting into salesman routes;
//   - a cluster model (ClusterInfoV6, ClusterModelV6, DesiredClusterCountV6)
//     with candidate-list augmentation using inter-cluster bridge edges
//     (AugmentCandidatesWithClusterBridgesV6).
// Documented in Appendix D as a NEGATIVE result: on n=25K/50K uniform grid portions it
// yields only ~25% valid runs — aggressive cluster-bridges break route connectivity.
// Kept in the repository as an illustration that trading too much quality for throughput
// + complicating the candidate structure can destroy validity. Registered as "lkh-wrapper-v6".
...
// Makes candidate lists symmetric: if b is in the candidate list of a, a is added to the list of b.
// The max_per_node cap prevents list bloat in dense regions.
// Symmetry is important for 2-opt filter correctness (without it, some moves would be missed).
...
// Accumulates edge appearance frequencies from a closed tour into an EdgeFreqMap.
// Each ILS iteration contributes edges from the current best route; edges that appear
// consistently accumulate high frequency and are considered the "backbone" of the solution.
template <typename DistanceFn>
...
// Greedy nearest-neighbor tour over a subset of vertices (sample).
// Used in POPMUSIC: each iteration takes a sample (~sqrt(n) vertices) and builds a short tour,
// whose edges are then contributed to the global edge-frequency map.
...
// Bounded 2-opt improvement of a closed tour (max_passes passes).
// Used as a lightweight polish inside POPMUSIC samples, where full 2-opt convergence
...
// Optimizes a fixed window within the tour as an open path (endpoints are fixed).
// Used in POPMUSIC: instead of full 2-opt over the entire tour, only a small segment is
// optimized — this gives linear complexity in the tour length.
template <typename DistanceFn>
...
// One FastPOPMUSIC pass: samples vertices from the tour, builds a local sub-tour,
// and optimizes it with OptimizeWindowOpenPathV5. A cheap analogue of Helsgaun's "full"
// POPMUSIC: O(sample_size^2) per pass, without rebuilding the candidate list.
template <typename DistanceFn>
...
// Main POPMUSIC tour factory: builds an initial nearest-neighbor tour,
// runs several FastPOPMUSIC passes to convergence or until budget cancellation,
// and returns the optimized tour. This is the foundation of the split-into-routes phase in v6.
template <typename DistanceFn>
...
// Description of a single cluster in ClusterModelV6.
// estimate — heuristic cost estimate for serving this cluster with one salesman:
// 2 * depot_distance (round trip) + spread_term (intra-cluster distances) + outlier_penalty.
// Used to distribute clusters among routes while minimizing total cost.
struct ClusterInfoV6 {
    std::vector<int> members;          // indices of clients belonging to the cluster
    Coord centroid{0.0, 0.0};          // cluster centroid (center of mass)
    double radius = 0.0;               // maximum distance from centroid to any member
    double avg_dist = 0.0;             // average distance from centroid
    double std_dist = 0.0;             // standard deviation of distances from centroid
    double depot_distance = 0.0;       // distance from centroid to the depot
    double estimate = 0.0;             // heuristic cost estimate (see above)
    int outlier_count = 0;             // number of clients beyond avg + 1.5*std
};

// Complete cluster model for the instance.
...
// Euclidean distance between two coordinates (with sqrt). A thin wrapper
// over SquaredDistanceCoordsV5 — used where the actual length is needed, not the square.
double DistanceCoordToPointV6(const Coord& lhs, const Coord& rhs) {
    return std::sqrt(SquaredDistanceCoordsV5(lhs, rhs));
}

// Heuristic cluster count selection: 6m + (n/12000), clamped to [4m, 12m].
// For n=100K, m=5: 6*5 + 8 = 38 clusters, clamped to [20, 60] -> 38.
// For n=10K, m=5: 6*5 + 0 = 30, clamped [20, 60] -> 30.
// Idea: cluster count is a multiple of the salesman count plus a density bonus.
...
// Lightweight k-means for building the cluster model. 4--5 iterations (depending on n),
// centroids initialized randomly from vertices. After convergence computes radius/avg/std/outliers
// and the heuristic cost estimate. This is a fast O(n*k*iterations) approximation of full k-means.
// Budget-aware: iteration is interrupted on cancellation and the partial model is returned.
...
// Safe insertion of a candidate into a candidate list (with duplicate and index checks).
...
// Sorts candidate lists by actual distance (not by index) and trims to max_per_node.
// Required after AugmentCandidatesWithClusterBridgesV6: cluster bridges are inserted
// without ordering, and this call restores the nearest-first order.
...
// The key idea in v6: augmenting candidate lists with inter-cluster bridge edges.
// For each cluster the 2 nearest clusters (by centroid distance) are found and edges
// are added between their representative vertices (closest to the target). Outlier vertices
// also receive additional candidates: the depot and representative vertices of neighboring clusters.
// The intent is to expose moves that the v3/v4/v5 candidate filter would miss
// (they would not appear in k-NN), but which could improve balance via bridges.
// In practice (see v6 results in Appendix D) this idea produces unstable routes:
// aggressive cluster-bridges break connectivity, which led to ~25% valid runs.
...
// Trims global candidate lists to the local_candidate_count nearest entries per vertex.
// Used in local search phases where retaining all inter-cluster bridges is unnecessary:
// global_sets holds the extended set (including cluster bridges), local holds only the top-k.

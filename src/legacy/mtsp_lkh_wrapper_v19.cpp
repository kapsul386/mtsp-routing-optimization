// v19: clean hybrid-native mTSP MINSUM solver.
// Seven key design decisions vs v17/v18 (see Appendix E):
//   1. Multi-seed portfolio selected strictly by MINSUM (no balanced-surrogate bootstrapping).
//      Seeds: round-robin nearest-neighbour, fast-lookahead, polar-sweep, savings (small n only).
//   2. Strict MINSUM-only inter-route acceptance. Optional cluster-aware rebalance
//      runs in a sandbox route set and is accepted only on strict MINSUM improvement.
//   3. Candidate-driven intra-route ILS + double-bridge.
//   4. Exhaustive final 2-opt polish on each route.
//   5. Best-MINSUM snapshot after each phase.
//   6. Anytime safety: roll back to snapshot if a phase regresses.
//   7. Budget split: seed <= 8%, candidate-set <= 12%, intra-route ILS ~35%, remainder for inter-route and polish.
...
// Ensures that every route starts and ends at the depot (vertex 0).
// If a route is open (e.g., after a repair phase), the missing 0s are appended.
// Used as a precondition after any ruin phase.
...
// Local validity check (takes node_count as a parameter rather than using the singleton Instance).
// Used inside the v19 pipeline where validation without a dependency on the global Instance is needed.
...
// Strict MINSUM relocate: tries to move a single client from route A to route B
// and accepts the move only if the total MINSUM length strictly decreases.
// Returns true on success (routes modified), false otherwise.
// This is one of the three canonical inter-route operations in v19 (relocate, swap, or-opt).
...
// Strict MINSUM swap: tries to exchange two clients between routes A and B
// and accepts only when the total MINSUM strictly decreases. Single best-move per call.
template <typename DistanceFn>
...
// Or-opt cross-route: tries to move a block of 2--3 consecutive clients from one route
// to another with a strict MINSUM decrease. Stronger than a single relocate — preserves
// the local block sequence, which often yields a larger edge saving. Single best-move per call.
template <typename DistanceFn>
...
// mtsp::ImproveRoute2Opt but uses the cached oracle.
// No candidate-list filtering (full O(L^2)) — yields a genuine 2-opt local optimum.
// From source comments: "this is the single biggest reason 2opt+greed beats v17/v18 at n=10k/25k".
// Budget polling in the inner loop ensures the deadline is respected even on huge routes.
template <typename DistanceFn>
    ...
    while (improved) {
        ...
        for (size_t i = 1; i + 2 < route.size(); ++i) {
            ...
            for (size_t j = i + 1; j + 1 < route.size(); ++j) {
                if ((j & 4095U) == 0 && budget.ShouldStop()) return;
...
// only practical way to reach a 2-opt local optimum in bounded time.
// Candidate-restricted (neighbor-list) 2-opt on a single route.
// Faster than Exhaustive2Opt for large L, but produces a less thorough optimum.
// Used as a pre-pass before exhaustive 2-opt in the final polish phase.
template <typename DistanceFn>
...
// OpenMP-parallel route polish. Each route is handled by a separate thread using
// IteratedLocalSearchV5 (candidate-driven 2-opt + double-bridge).
// All threads share a common budget deadline; every thread must stop when it expires.
...
// v19: clean hybrid-native MINSUM mTSP solver. Registered as "lkh-wrapper-v19".
// Pipeline (see header comment, 7 key design decisions):
//   1. Multi-seed portfolio (NN/lookahead/polar/savings) — selected by MINSUM;
//   2. Candidate-driven intra-route ILS;
//   3. Strict MINSUM inter-route exchange (relocate / swap / or-opt);
//   4. Exhaustive 2-opt polish on each route;
//   5. Best-MINSUM snapshot after each phase;
//   6. Anytime safety: roll back to snapshot on regression;
//   7. Budget split: seed <= 8%, candidate <= 12%, intra <= 35%, polish + inter — remainder.
class LkhWrapperSolverV19 : public mtsp::Solver {
public:
...
static bool reg_lkh_mtsp_v19 = (SolverFactory::RegisterSolver("lkh-wrapper-v19", []() {
    return std::make_unique<LkhWrapperSolverV19>();
}),
                                true);

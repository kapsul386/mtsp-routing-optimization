...
//   1. Destroy: remove K random customers from the current best routes.
//   2. Recreate: reinsert each customer at the candidate-restricted position
//      that minimises insertion delta, across all m routes.
//   3. Cleanup: candidate-based 2-opt on the m affected routes.
//   4. Accept: only if MINSUM strictly improves (hill-climber). Otherwise
//      revert to the snapshot.
// Repeat until time budget exhausts. The strict-improvement acceptance keeps
// the safety-net guarantee: v21 MINSUM <= v20 MINSUM <= 2opt+greed MINSUM.
//
...
// One LNS iteration: destroy K customers via spatial cluster destroy, greedily
// reinsert each at the candidate-restricted position that minimises insertion
// delta, then run a 2-opt cleanup on the m affected routes. Mutates `routes`
// in place. Returns the resulting MINSUM, or +inf on aborted iteration. The
// caller decides whether to accept (e.g. SA Metropolis).
template <typename DistanceFn>
